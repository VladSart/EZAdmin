...
**Not covered:** Microsoft Defender for Cloud Apps session policies (a different product, sometimes confused with GSA due to overlapping "access" terminology), macOS GSA client-specific installation/architecture/troubleshooting (system extension + transparent proxy activation, MDM allow-listing, the June 2025 bundle-identifier migration, macOS 26 compatibility — see `macOS/Troubleshooting/GlobalSecureAccess-macOS-A.md`/`-B.md`), Android/iOS GSA client specifics (each has separate deployment mechanics); Windows GSA client-specific installation/services/health-check mechanics, the November 2026 Windows Update auto-upgrade delivery model, and the Prefer Local Network overlapping-subnet fix — see `Windows/Troubleshooting/GlobalSecureAccess-Windows-A.md`/`-B.md`, legacy on-prem App Proxy-only publishing (see `EntraID/Troubleshooting/AppProxy-B.md` — GSA Private Access and App Proxy are related but distinct publishing mechanisms that can coexist), **licensing and cost — what each tier unlocks, Suite vs. standalone Internet Access/Private Access pricing, the guest Monthly Active User billing model, and cohort-based cost-allocation guidance (see `EntraSuiteLicensing-A.md`/`-B.md`; this file assumes licensing is already correctly in place and focuses purely on client/connector/policy technical behavior)**.
