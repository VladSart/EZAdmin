...
- **macOS has its own client architecture and known-issue set**: system extension + transparent application proxy activation (two independently-gated components), a June 2025 bundle-identifier migration, and a mandatory client-version floor before any macOS 26 upgrade — see `macOS/Troubleshooting/GlobalSecureAccess-macOS-B.md`/`-A.md` rather than troubleshooting a Mac-reported GSA issue as if it were the Windows client.
