...
1. **Confirm which provider(s) are affected**: Entra roles, Azure resources or Groups. *(Corrected in run 254.)* The Learn pages only name `aadRoles` and `azureResources`, but MC1181281 says Iteration 2 calls "for Azure resources, Microsoft Entra roles **and Groups** will fail". Treat any legacy `/beta/privilegedAccess/aadGroups` caller as in scope. The Iteration 3 Groups path `/identityGovernance/privilegedAccess/group/...` is **not** affected. Don't let a loose `privilegedAccess` grep flag it.
...
- *(Corrected in run 254.)* Server-side call telemetry **does** exist if the `MicrosoftGraphActivityLogs` diagnostic setting is enabled: `MicrosoftGraphActivityLogs | where RequestUri has "/beta/privilegedAccess/"` returns the calling `AppId` and `UserId` (see [PIMIteration2Retirement-A.md](PIMIteration2Retirement-A.md) Validation step 3). Without it, no usable server-side call telemetry exists for "who is still calling Iteration 2" — the practical proxy is auditing **legacy permission grants** (`PrivilegedAccess.*` scopes) combined with a source-code/flow-definition grep, which is what this runbook's Triage section and `Get-PIMIteration2APIUsageAudit.ps1` (see `EntraID/Scripts/`) both do. Treat the result as a lead list requiring manual confirmation, not an authoritative inventory.
...
- Full deep dive: [PIMIteration2Retirement-A.md](PIMIteration2Retirement-A.md), which covers the Iteration 2 → 3 `type`→`action` mapping, the ARM auth change, Groups, and AzGovViz as a known affected tool.
