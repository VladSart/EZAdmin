...
## Build Progress
- Total files (real count via `find`, excludes `_BUILD/` and `.git/`): 500
- Completed: 500
- In progress: 0
- Queued: 0
- Last updated: 2026-07-07 (auto-build, run 50, scheduled task "ezadmin-day-build": manifest queue still empty, only the legend row, no actual ⬜ items. Read run 49's own "For next run" list directly — it named two immediate follow-ups within `ActiveDirectory/`: Group Policy processing/replication and AD DS backup/restore (authoritative vs. non-authoritative restore, USN rollback detection), plus older carryovers from run 47 (Sentinel analytics rules, Logic Apps playbooks, Azure Arc, Defender for Cloud CSPM, Workload Identity Federation). Picked **AD DS backup/restore** over GP-processing because it's a materially higher-severity gap: confirmed via `grep -ril "authoritative restore\|USN rollback\|ntdsutil"` that zero files anywhere in the repo covered backup/restore, DSRM, or USN rollback (a GPO-processing-under-replication topic would at least partially overlap with the existing `Windows/Troubleshooting/GPO-B.md` and `DFS/Troubleshooting/Replication/`, whereas backup/restore was a completely unbuilt failure mode with no fallback content anywhere in the domain). Built `Troubleshooting/BackupRestore/AD-BackupRestore-B.md` (hotfix) leading triage with `wbadmin get versions`, a Directory Service event log scan for IDs 2095/1113/1115 (USN rollback family), and tombstone-lifetime comparison, with 4 fix paths (isolate-and-rebuild for USN rollback, Recycle-Bin-first scoped authoritative restore, DSRM password reset, and a hard stop-gate for backups older than tombstone lifetime) — explicitly framed USN rollback as non-repairable-in-place and the stale-backup fix as a decision gate rather than an executable remediation, since restoring a stale backup causes active harm (lingering objects) rather than just failing safely. Built `Troubleshooting/BackupRestore/AD-BackupRestore-A.md` (deep dive) covering System State backup internals (VSS/AD-aware writer, why VM snapshot rollback of a live DC is unsupported), the authoritative-vs-non-authoritative restore distinction and why version-number incrementing is what makes authoritative restore "win" against normal replication, full USN rollback mechanics (invocationID vs. USN, why it causes silent skipped replication rather than an obvious error, and why Server 2003 SP1+ auto-disables outbound replication on detection), DSRM as a credential-rotation concern, and AD Recycle Bin as the preferred first-line recovery path; includes a 9-layer dependency stack, a 10-row symptom→cause map, 7 validation steps, a 5-phase troubleshooting flow, and 3 remediation playbooks (USN-rollback demote-and-rebuild, scoped OU authoritative restore, non-authoritative rebuild-restore) — explicitly scoped full forest recovery as out-of-scope and pointed to Microsoft's dedicated Forest Recovery guide rather than attempting to compress it into this single-DC-focused runbook. Built `Scripts/Get-ADBackupRestoreHealth.ps1` — read-only, parses `wbadmin get versions` for the most recent backup timestamp and compares its age against `tombstoneLifetime` (flagging ERROR past 100% and WARN past 75% of that window), checks the NTDS VSS writer state via `vssadmin list writers`, scans the Directory Service event log for 2095/1113/1115 (USN rollback family, ERROR) and 1988 (lingering objects, WARN, explicitly cross-referenced to the Replication runbook rather than this one), checks `repadmin /options` on the local DC for pre-existing `DISABLE_OUTBOUND_REPL`/`DISABLE_INBOUND_REPL` flags (a common leftover sign of an unresolved prior USN rollback), and reports AD Recycle Bin enablement; exports one consolidated CSV. Verified brace/paren balance via `grep -o` counts before committing (81/81 braces, 75/75 parens — no `pwsh` runtime available in this sandbox for a real parse check, consistent with every prior run's caveat). Confirmed no destructive cmdlets (`wbadmin start systemstaterecovery`, `ntdsutil`, `Uninstall-ADDSDomainController`, `Enable-ADOptionalFeature`, etc.) appear anywhere in the script — those exist only inside the two runbooks' documented Fix Paths/Playbooks, consistent with the established repo-wide pattern of scripts staying strictly read-only. Updated `ActiveDirectory/_AGENT.md`: revised the top summary line to mention backup/restore, added three new folder-contents rows, six new "Common entry points" bullets, and a third, clearly-labeled "Backup/restore dependency chain" ASCII diagram separate from both the replication and trust chains (this one runs through VSS/backup-age/DSRM/ntdsutil rather than network/KCC/USN-exchange or DNS/TDO/SID-filtering). Did not touch `AGENT_INDEX.md` this run, matching run 49's rationale — the Active Directory domain row already generically covers the folder and no new cross-domain relationship was introduced beyond what's already cross-referenced inside `_AGENT.md` (GPO/SYSVOL note pointing to `DFS/` and `Windows/Troubleshooting/GPO-B.md`). Confirmed via `ls .git/*.lock*` that only `.stale`/`.bak`-suffixed files remain (harmless, flagged since run 38 as an outstanding one-time human cleanup task, not actioned) — no live lock files blocking this run's commit. **For next run:** Group Policy processing/replication (the SYSVOL-plus-AD-replication interaction, distinct from client-side `Windows/Troubleshooting/GPO-*.md`) is now the last obvious gap to round out the `ActiveDirectory/` domain to three well-covered topics (replication, trusts, backup/restore) plus GPO processing as a fourth. Outside AD, run 47's carryovers remain valid and increasingly stale: Sentinel analytics-rule/false-positive tuning, Logic Apps playbook failures, Azure Arc-enabled server onboarding, Microsoft Defender for Cloud (CSPM), and Workload Identity Federation — worth prioritizing one of these soon so the repo doesn't over-index on a single domain for five consecutive runs. The misfiled-script bucket (`Autopilot-Troubleshooting2.ps1`, `Autopilot-Network-Connectivity.ps1`, `Intune/IntuneChecker.ps1`, `Autopilot/Scripts/Test-AutopilotNetworkRequirements.ps1` vs. `Autopilot/Troubleshooting/Autopilot-Network-Connectivity.ps1`) remains open since runs 30-49 and still needs interactive user review, not autonomous action. The renamed `.stale*`/`.bak*` git lock file cleanup flagged since run 38 remains an outstanding one-time human task.
