...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 11: manifest queue still empty. Continued directly on run 10's own recommendation — "Remaining Defender gaps: CloudProtection, DefenderVulnMgmt, MDA, MDI, NetworkProtection — suggest these next, since MDI/CloudProtection also see frequent ticket volume." Built the three highest-priority of those five: `Get-CloudProtectionStatus.ps1` (MAPS/cloud-delivered-protection state, policy-source check via the SpyNetReporting registry key so techs can immediately see whether a GPO/Intune policy is overriding a local fix, cloud endpoint reachability, and signature freshness), `Get-MDIStatus.ps1` (the first Defender script in this repo written DC-scoped rather than endpoint-scoped, since MDI is a sensor-per-Domain-Controller architecture with no meaningful "local device" health check — auto-discovers DCs via the ActiveDirectory module when no target list is given, and checks AATPSensor state, workspace connectivity, the four audit-policy subcategories MDI depends on, and RestrictRemoteSam — a documented silent killer of lateral-movement detection), and `Get-DefenderVulnMgmtStatus.ps1` (MDVM has no sensor of its own, so this checks the MDE/Sense prerequisites MDVM rides on top of — onboarding state, Win32_Product count as a WMI-corruption proxy for software inventory failures, and Compatibility Appraiser task recency). All three explicitly scope out what they don't cover in NOTES (MDI portal-side health alerts, MDVM exposure score/CVE data, and MDVM license validation are all cloud/Graph-side and were left to the corresponding -B.md runbooks) and cross-reference their runbook pairs. Remaining Defender script gaps: MDA, NetworkProtection (2 of the original 5 named topics). Checked for stale `.git/HEAD.lock`/`index.lock` per the standing environment note before committing.
