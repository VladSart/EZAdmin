...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 6: manifest queue still empty; continued the script-coverage gap-check pattern for a sixth consecutive run. Verified on disk (directory listing before building, no existing files overwritten) that Windows/Scripts was still missing companion scripts for DeliveryOptimization, UserProfile, and WMI — the three topics flagged as remaining at the end of run 5. Built `Get-DeliveryOptimizationDiagnostics.ps1` (DoSvc service state, effective DODownloadMode/GroupId/CacheHost/cache-size policy values with plain-English mode labels, per-file peer/cache/HTTP transfer stat aggregation with a specific check for the "config looks right but zero peer bytes" misdiagnosis pattern the runbook calls out, DO firewall rule state, and optional port 7680 peer / port 443 Connected-Cache reachability tests via -PeerToTest/-CacheHostOverride params), `Get-UserProfileDiagnostics.ps1` (current-session temp-profile path detection, SID resolution + ProfileList registry state with State-value interpretation table matching the runbook's Learning Pointers (0/4/256/1024), duplicate SID/.bak key detection — the runbook's most-common-cause fix — NTUSER.DAT and profile-folder-on-disk presence checks, recent User Profile Service event log query for IDs 1500/1502/1505/1511/1515/1530/1534, and C: free space threshold check), and `Get-WMIDiagnostics.ps1` (winmgmt service state, basic Get-CimInstance query test, winmgmt /verifyrepository consistency check, recent WinMgmt Application-log error aggregation, disk space corruption-risk check, and a read-only root\subscription __EventFilter/__EventConsumer/__FilterToConsumerBinding inventory exported to a companion CSV for security review — explicitly never auto-removes subscriptions per the runbook's escalate-before-touching guidance). All three follow the established comment-based-help/doc-header + Write-Status-or-record + Preflight→Detect→Execute→Validate→Report + CSV-export pattern and cross-reference exact Fix numbers from their `-B.md`/`-A.md` runbooks. Checked for stale `.git/index.lock`/`HEAD.lock` per the standing environment note — found both, cleared via rename (not delete) before committing, consistent with prior runs' documented workaround. **Windows/Scripts script coverage gap is now fully closed** — every Windows/Troubleshooting topic with a -B.md runbook has a companion script. Remaining script-less topics: ~5 on macOS (Extensions, Compliance-Policies, MDM-Certificate-Renewal, ABM-Token-Renewal, Shell-Script-Failures) — good material for the next pass, along with the manifest bookkeeping reconciliation flagged in prior runs.
