...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 5: manifest queue still empty; continued the script-coverage gap-check pattern for a fifth consecutive run. Verified on disk that Windows/Scripts and macOS/Scripts were both still missing companion scripts for CredentialManager, CertificateServices, and PPPC respectively (confirmed via directory listing before building — no existing files overwritten). Built `Get-CredentialManagerDiagnostics.ps1` (VaultSvc/CryptSvc service state, cmdkey /list stored-credential enumeration with duplicate-target and Office/OneDrive/SharePoint entry detection, Kerberos ticket cache health via klist with TGT presence check, per-user DPAPI master key folder presence check, client-side LmCompatibilityLevel, and Test-ComputerSecureChannel for NetLogon trust health — deliberately never captures credential values, matching the runbook's redact-passwords guidance), `Get-CertificateServicesDiagnostics.ps1` (LocalMachine\My cert enumeration with expiry-window flagging, X509Chain build/validation per cert with RevocationStatusUnknown detection, auto-enrollment Event ID 19/6 lookup, CDP URL extraction + live HTTP reachability test, optional CA server RPC-135/HTTPS-443 reachability via -CAServerFQDN param, NTAuth store enumeration, and certsvc state check that self-identifies as N/A on non-CA client machines), and `Get-PPPCStatus.sh` (MDM supervision/DEP status, bundle-ID-or-app-path resolution via mdfind/mdls, codesign -dv identifier and Team ID extraction with bundle-ID-mismatch warning, TCC.db query by resolved bundle ID with auth_value/auth_reason interpretation distinguishing MDM-granted vs. user-granted vs. denied, and installed-PPPC-payload count via profiles show -all). All three follow the established comment-based-help/doc-header + Write-Status-or-record + Preflight→Detect→Execute→Validate→Report + CSV-export pattern and cross-reference exact Fix numbers from their `-B.md`/`-A.md` runbooks. No git lock issues encountered this run (checked for stale `.git/index.lock`/`HEAD.lock` per the standing environment note — none found). Remaining script-less topics after this run: ~4 on Windows (DeliveryOptimization, UserProfile, WMI — CertificateServices and CredentialManager now covered) and ~5 on macOS (Extensions, Compliance-Policies, MDM-Certificate-Renewal, ABM-Token-Renewal, Shell-Script-Failures — PPPC now covered) — good material for the next pass.
