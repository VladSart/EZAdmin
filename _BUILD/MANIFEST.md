...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-07 (auto-build, run 40, scheduled task "ezadmin-night-build": manifest queue still empty (only the legend row, no actual ⬜ items). Per run 39's explicit "For next run" pointer, closed the last confirmed content gap from that thread: Communication Compliance (Purview), which previously had only two tangential passing mentions inside `Insider-Risk-A.md`/`eDiscovery-A.md` and no dedicated topic. Before trusting a quick topic-count-vs-script-count heuristic across the whole repo (DFS showed 5 topics/4 scripts, M365/Licensing showed 2 topics/1 script), re-verified both by reading the actual script SYNOPSIS blocks first — `DFS/Scripts/Get-DFSNamespaceConfigAudit.ps1`'s own docstring confirmed it already covers BOTH ABE and SiteCosting topics (namespace-vs-share ABE mismatch audit + referral priority override audit), and `M365/Licensing/Scripts/Get-LicenseReport.ps1`'s docstring confirmed it already covers both License-Assignment and Group-Based-Licensing — both were false positives from the heuristic, not real gaps, consistent with the standing lesson (runs 27-39) that these counts must be verified against actual file content, not just tallied. Researched current Microsoft Learn documentation directly rather than relying on training-data recall, since Communication Compliance's specific details (the explicit, twice-repeated "PowerShell isn't supported for creating and managing Communication Compliance policies" statement, the 7 policy templates and their exact review percentages — notably *Regulatory compliance* and *Sensitive information* default to 10% while every other template defaults to 100%, the 5 named role groups plus 4 equivalent Entra/Purview roles, the reviewer-vs-scoped-user group-type support asymmetry, Viva Engage Native Mode gating, non-Microsoft AI pay-as-you-go billing gating, and the ~24h/~48h processing-latency split by channel) are exactly the kind of precise detail liable to be misremembered: fetched `communication-compliance` (updated 2026-06-01), `communication-compliance-policies`, `communication-compliance-plan` (updated 2025-11-18), `communication-compliance-permissions` (updated 2026-06-01), and `communication-compliance-configure` in full, plus a targeted search confirming the `AllowSecurityEndUserReporting` property on `Get-CsTeamsMessagingPolicy` as the real, correct cmdlet/property for the Teams self-report toggle before using it in the script. Noted and preserved (rather than silently resolving) a genuine contradiction between two MS Learn pages of different dates regarding whether mail-enabled security groups are supported for policy scoping — flagged explicitly in `CommunicationCompliance-A.md`'s How It Works section as tenant/version-dependent rather than picking one source arbitrarily. Built the full three-file pattern: `Security/Purview/CommunicationCompliance-B.md` (hotfix — triage built entirely around adjacent/proxy checks since no cmdlet exists for the core object, dependency cascade from licensing through audit log through role groups through channel/scoping prerequisites to processing latency, 5 fix paths covering zero-admin lockout recovery — including the non-obvious fact that Global Admins have NO default access to this solution — reviewer eligibility, licensing, and portal-only condition-testing guidance for a configured-but-not-matching policy), `Security/Purview/CommunicationCompliance-A.md` (deep dive — full workflow architecture Configure→Investigate→Remediate→Maintain, the legacy "Supervisory Review" naming that's still operationally load-bearing in the `SupervisoryReview{*}` mailbox pattern used by compliance boundaries, a 7-row policy template table with exact locations/direction/review-%/conditions per template, the three-different-rulesets scoping model (scoped/excluded/reviewer), a 5-role-group x 4-capability matrix, dependency stack, 12-row symptom→cause map, 6 validation steps, 6-phase troubleshooting flow, 4 remediation playbooks including the officially-documented bulk distribution-group provisioning pattern and the compliance-security-filter workaround for the `SupervisoryReview` mailbox pattern, a multi-section evidence-pack script, 10-row command cheat sheet, and 6 learning pointers), and `Security/Purview/Scripts/Get-CommunicationComplianceReadinessAudit.ps1` (read-only adjacent-signal audit — explicitly does NOT and cannot report on policy configuration/matches/alerts since no API surface exists for that; instead audits Unified Audit Log state, all 5 CC role groups' membership with an explicit zero-admin-lockout flag, per-reviewer EXO-mailbox-hosting + Analysts/Investigators role-group membership for a supplied UPN list, per-user CC-qualifying licence/service-plan presence via `Get-MgUserLicenseDetail`, Teams `AllowSecurityEndUserReporting` state across every messaging policy since that feeds the CC "User-reported messages" system policy, and an optional `-CheckComplianceBoundaryFilter` switch that checks for an existing `New-ComplianceSecurityFilter` covering the `SupervisoryReview{*}` pattern). All read-only — no policy/role-group/licence/filter create-update-delete calls anywhere in the script. Verified brace/paren/bracket balance via Python counts before committing (65/65 braces, 83/83 parens, 26/26 brackets; no pwsh available in this sandbox for a real parse check, consistent with every prior run's caveat). Backfilled `Security/Purview/_AGENT.md` — this one was NOT significantly stale (unlike several other domains flagged in runs 32-39), just missing the two new CommunicationCompliance rows, four new entry-point rows, and three new diagnostic commands, which were added in place rather than requiring a full rewrite. Also added a Domain Map row to `AGENT_INDEX.md` for Communication Compliance, cross-referencing `M365/Exchange/`, `M365/Teams/`, and `EntraID/`. Checked `.git/*.lock` for a live (non-`.bak`/`.stale`-suffixed) lock file before committing. **For next run:** the `_AGENT.md` staleness check flagged since run 38 for Intune, Windows, and M365/Exchange (all past 20+ files per folder) still has not been run — this is now the oldest open item in this thread and should be prioritized next since it's been deferred for two runs in a row. The three misfiled/non-conforming Autopilot files and the broader "manifest bookkeeping behind repo state" item remain open since runs 30-39 and still need interactive user review, not autonomous action. No further genuinely new dedicated-topic gaps were identified this run — the repo's topic coverage across all domains now appears comprehensive; future runs should likely shift focus toward the deferred `_AGENT.md` staleness backlog and script-coverage spot-checks rather than new-topic hunting.
