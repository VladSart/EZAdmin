...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 10: manifest queue still empty. Picked up directly on the prior run's own recommendation — "suggest continuing with WHfB, SSPR, and PRT-Issues in EntraID" — since EntraID had 14 Troubleshooting topics but was still missing companion scripts for exactly those three. Built `Get-WHfBRegistrationStatus.ps1` (tenant-wide WHfB key registration coverage, cross-referenced against each user's owned-device join type to flag users with Hybrid Joined devices and zero WHfB keys — the highest-risk combination for Cloud Kerberos Trust misconfiguration), `Get-SSPRCoverageReport.ps1` (registered-method-count compliance report that correctly special-cases admin accounts, which cannot use security questions and are held to a stricter non-question-method bar), and `Get-PRTFleetRisk.ps1` (fleet-wide triage tool that flags devices at risk of PRT failure via proxy signals — disabled state, stale sign-in beyond the 14-day PRT renewal window, hybrid-joined + stale combination — since PRT itself isn't visible via Graph and per-device dsregcmd evidence collection doesn't scale to a full tenant sweep). All three explicitly document their own limitations in NOTES (Cloud Kerberos Trust server object and true SSPR policy method-count aren't Graph-visible; PRT fleet risk is inferred, not confirmed) and cross-reference the existing WHfB-A.md/SSPR-A.md/PRT-Issues-A.md runbooks for the per-user/per-device follow-up steps they don't replace. Checked for stale `.git/HEAD.lock`/`index.lock` per the standing environment note before committing.
