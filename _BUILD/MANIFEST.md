...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 15: manifest queue still empty. Followed run 14's explicit recommendation — Intune has 19 script-less topics, "now the single largest topic-to-script ratio gap in the repo and the clear priority for the next run." Closed the 3 highest ticket-volume of those 19: `Get-AppDeploymentDiagnostics.ps1` (device-local IME service/process/task health, IME + AgentExecutor log error-pattern counts, Delivery Optimization stalled-transfer detection, detection-rule-relevant installed-app inventory via both native and WOW6432Node uninstall registry keys, and free disk space — consolidates the ~6 checks App-Deployment-A.md's Symptom → Cause Map scatters across 10 failure signatures into one pass, same remote/local dual-mode pattern as the existing Get-EnrollmentDiagnostics.ps1), `Get-PolicyConflictScan.ps1` (the first **fleet-scale** complement to Policy-Conflict-A.md's device-local Evidence Pack — uses Graph deviceConfigurations/configurationPolicies deviceStatuses endpoints to enumerate every device+profile pair tenant-wide currently in Conflict or Error, ranks profiles by TotalIssues so engineers know which profile to open first per Policy-Conflict-A.md Phase 2's conflict-type triage, rather than clicking through every profile individually in the portal), and `Get-AutopatchReadiness.ps1` (fleet-level ring audit that specifically targets the "Device duplicated across two rings" failure Autopatch-A.md's Symptom → Cause Map and Playbook 2's rollback note call out — cross-references registration group membership against all four ring groups to flag dual-ring devices, flags devices in zero rings, flags rings below a configurable MinRingSize per Phase 3's statistical-significance guidance, and flags stale Entra sign-in as a pre-check before chasing readiness failures per Phase 2 guidance not to troubleshoot readiness on a device that may simply be offline). All three read-only. **Intune script coverage is now 12/22** (9 pre-existing + 3 this run; remaining script-less: AppProtection, CoManagement, CustomCompliance, DriverManagement, EPM, FeatureUpdates, Filters, GP-to-CSP, Kiosk, Managed-Apps, Platform-Scripts, Remediations, ScopeTags, WUfB — Security-Baselines and Certificates already have scripts and were not double-counted). Checked for stale `.git/index.lock`/`HEAD.lock` per the standing environment note before committing; none found this run.
