...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 7: manifest queue still empty; continued closing the macOS script-coverage gap flagged at the end of run 6. Verified on disk that macOS/Scripts had zero companion scripts for Extensions, Compliance-Policies, and MDM-Certificate-Renewal (all three have complete -B.md/-A.md runbooks). Built `Get-SystemExtensionStatus.sh` (systemextensionsctl state per extension, SIP status, MDM supervision/ADE check, com.apple.system-extension-policy and legacy kernel-extension-policy profile detection, Apple Silicon vs Intel architecture note, duplicate-EndpointSecurity-type conflict detection, recent com.apple.system_extensions log errors), `Get-ComplianceStatus.sh` (MDM enrollment, FileVault/Firewall/Gatekeeper/SIP/Secure-Boot-on-Apple-Silicon state — the exact signal set Intune's compliance engine reads per the runbook's Dependency Stack — plus Company Portal presence/version and IME daemon + log check for custom-attribute compliance), and `Get-MDMCertificateStatus.sh` (MDM enrollment state, device-side MDM identity certificate expiry parsed from the System keychain with a days-remaining threshold, a live non-destructive `mdmclient CheckIn` trigger matching the runbook's own Diagnosis Step 4, Intune enrollment-endpoint and Apple APNs gateway reachability tests on 443/2195, SSL-inspecting-proxy detection since that's the runbook's most-cited root cause, and recent mdmclient log errors). All three follow the established `record()`/CSV-export/summary pattern seen in `Get-PPPCStatus.sh` and `Get-PlatformSSOStatus.sh`, are read-only except the single documented check-in trigger, and cross-reference exact Fix/Playbook numbers from their `-B.md`/`-A.md` runbooks. Checked for stale `.git/index.lock`/`HEAD.lock` per the standing environment note — found a stale `HEAD.lock`, cleared via rename (not delete) before committing, consistent with prior runs' documented workaround. **Two macOS topics remain script-less: ABM-Token-Renewal and Shell-Script-Failures** — good material for the next pass, along with the manifest bookkeeping reconciliation flagged in prior runs (still not actioned, still not blocking).
