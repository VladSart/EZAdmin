...
- **Script coverage gap now confirmed to extend well beyond Windows/macOS** (which runs 5-7 closed out). As of this run: EntraID has 14 Troubleshooting topics but only 8 scripts (AccessPackages, AppProxy, CAE, DynamicGroups, IdentityProtection, PRT-Issues, PasswordProtection, SSPR, WHfB still have none); Security/Defender has 9 topics but only 4 scripts (ASR-Rules and Tamper-Protection closed this run — CloudProtection, DefenderVulnMgmt, MDA, MDI, NetworkProtection, WDAC-adjacent topics still gapped beyond WDAC itself); Intune has 20 topics but only 9 scripts; M365/Exchange has 8 topics but only 3 scripts; SharePoint-OneDrive has 3 topics but only 1 script; Teams has 4 topics but only 1 script. This run closed the two highest-value Defender gaps (ASR, Tamper Protection) and the highest-value EntraID gap (MFA) since these are the most common MSP escalation topics. Remaining gaps are good material for future runs — suggest continuing with WHfB, SSPR, and PRT-Issues in EntraID, and MDI/CloudProtection in Defender, since those also see frequent ticket volume.
