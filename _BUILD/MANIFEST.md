...
| File | Status | Assigned |
|------|--------|----------|
| `ActiveDirectory/Troubleshooting/ADFS/DKMACLHardening-B.md` | ✅ | auto-build |
| `ActiveDirectory/Troubleshooting/ADFS/DKMACLHardening-A.md` | ✅ | auto-build |
| `ActiveDirectory/Scripts/Get-DKMACLHardeningAudit.ps1` | ✅ | auto-build |
| `ActiveDirectory/_AGENT.md` (backfilled — 1 new overview clause, 3 new Folder-contents rows, 3 new Common-entry-point bullets) | ✅ | auto-build |
| `AGENT_INDEX.md` (1 new row inserted directly after the existing AD FS / Web Application Proxy row) | ✅ | auto-build |

_2026-09-03 (run 230, scheduled task "ezadmin-night-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `bf6e674` (run 229, Teams Call Queue Automatic Recording + Purview DLP for Non-Microsoft Connected Apps) with no concurrent-run conflicts; file count 1,208. Also re-inspected the mounted working tree's own `git status`, which continues to show the large modified/untracked-file FUSE-staleness pattern first flagged in run 188 — the mount's local HEAD remains far behind `origin/master` (run 229's writeup already flagged a 60+ run gap, at HEAD `f374c6e`/run 169; this run did not re-measure the exact node but confirms the pattern persists and has not self-corrected across at least two subsequent runs). Per standing practice, did not attempt to reconcile the mount and authored all new work directly against the up-to-date `/tmp` scratch clone; a `diff -rq` between the mount and the fresh clone confirmed every file that differs or is "only in scratch" reflects the mount simply lagging on already-pushed run history, with zero genuinely new local-only work found — consistent with every run since 188. **Flagging directly for the user:** this mount-vs-origin lag is now persistent across many runs rather than a one-off staleness blip; if it does not close on its own, the FUSE mount refresh itself (outside this task's control) may need manual attention rather than continued content-authoring workarounds. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line and narrative-prose mentions of that phrase match the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Microsoft Entra, Microsoft Defender for Endpoint, and the Windows IT Pro Blog for August-September 2026, cross-checking every surfaced candidate against the repo via grep before investigating further: Windows Autopilot device association GA, the STIG audit baseline, Multi Admin Approval's Graph-API-automation extension, Administrator Protection (Windows 11), the App Settings/declarative binary-management DDM feature for macOS 27, Defender's AI-agent posture-risk scoring (already covered under `LocalAIAgentDiscovery-A.md`'s licensing section), and Windows Search/taskbar UI changes (judged too cosmetic for this repo's break-fix/hardening focus) were all either already confirmed covered via grep or explicitly out of scope. One genuinely new, high-value, time-sensitive candidate was built instead, live-fetched directly from the primary Microsoft Support source before writing anything. **AD FS DKM (Distributed Key Manager) Container ACL Hardening**, tracked as CVE-2026-56155 and documented in KB5121391 (`ms.date`/original publish 2026-07-14) — confirmed via repo-wide grep as a complete gap despite this repo's existing, extensive `ADFS-A/B.md` certificate/relying-party-trust coverage, which this topic sits *underneath* rather than duplicates. Live-fetched the full KB5121391 article directly (`support.microsoft.com`) rather than relying on search-summary snippets, given the topic's registry-key/event-ID precision requirements. Built around the single highest-value framing: this is a **phased hardening rollout with a hard, already-announced enforcement date** (July 14, 2026 Audit-mode detection start; October 13, 2026 Enforcement-mode default-remediation) that is currently mid-window as of this run's own date (2026-09-03 — roughly 40 days before Enforcement), making proactive opt-in remediation during the Audit-mode window the clearly-recommended path over waiting for automatic Enforcement to hit every unpatched farm simultaneously on the same date. Also built around the Golden-SAML-adjacent risk model (an overly permissive DKM container ACL lets any principal with read access decrypt the token-signing certificate's private key entirely offline, bypassing MFA/Conditional Access via forged tokens — the same impact class as a stolen signing certificate, delivered via ACL misconfiguration instead of key exfiltration), the five-event diagnostic model (1132 ACL-needs-review / 1133 healthy / 1134 detection-error / 1135 remediation-success-with-rollback-SDDL / 1136 remediation-failed), and the permanent Windows Server 2012/2012 R2 carve-out from automatic Enforcement (these platforms require a one-time manual `WriteOwner`/`WriteDacl` grant to the AD FS service account before opt-in remediation can succeed, and never receive the Enforcement-mode default regardless of patch level — a concrete forcing function for overdue OS-upgrade conversations on any farm still running them). Built `Get-DKMACLHardeningAudit.ps1` as a read-only, multi-node-capable audit script — per-node patch-level phase classification (pre-Audit / Audit / Enforcement, based on hotfix install date relative to the two milestone dates), a farm-wide (checked-once) live DKM container ACL read via the `AD:` PSDrive with a heuristic secure-baseline match check, current `RemediateDkmAcl` registry state per node, and the 10 most recent 1132-1136 events — explicit in its own header and inline comments that the baseline-match heuristic cannot reliably distinguish a legitimate AD FS service account ACE from a genuinely unexpected extra principal without per-environment service-account-name context, and that it performs zero write operations (no ACL changes, no registry changes, no service restarts) by design. Backfilled `ActiveDirectory/_AGENT.md` (1 new overview-paragraph clause inserted directly after the existing AD FS / WAP clause, 3 new Folder-contents rows after the existing `ADFS-A.md` row, 3 new Common-entry-point bullets after the existing AutoCertificateRollover bullet) and inserted 1 new `AGENT_INDEX.md` row directly after the existing AD FS / Web Application Proxy row, via targeted Python find-and-insert/string-replace on the up-to-date scratch clone, matching runs 190-229's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (18/18 in DKMACLHardening-B.md, 12/12 in DKMACLHardening-A.md — both even) and `<details>`/`</details>` tag-pair balance verified (7/7, 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass (62/62 braces, 87/87 parens, 20/20 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up — the persistent 60+ run mount lag flagged by run 229 and re-confirmed here should be surfaced to the user proactively if it still hasn't self-corrected. This topic carries a live deadline: if a future run's "today" date is on or after October 13, 2026, revisit both `DKMACLHardening-A/B.md` to update the "still in Audit mode" framing to reflect Enforcement having already begun, and confirm no clarifying follow-up KB was published between now and then. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Windows + macOS + Teams Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._

## New Topics — SharePoint/OneDrive Hero Link Sharing + Windows Post-Quantum Hybrid TLS (run 231)
| File | Status | Assigned |
|------|--------|----------|
| `M365/SharePoint-OneDrive/HeroLinkSharing-B.md` | ✅ | auto-build |
| `M365/SharePoint-OneDrive/HeroLinkSharing-A.md` | ✅ | auto-build |
| `M365/SharePoint-OneDrive/Scripts/Get-HeroLinkSharingAudit.ps1` | ✅ | auto-build |
| `Windows/Troubleshooting/PostQuantumTLS-B.md` | ✅ | auto-build |
| `Windows/Troubleshooting/PostQuantumTLS-A.md` | ✅ | auto-build |
| `Windows/Scripts/Get-PQCTlsReadinessAudit.ps1` | ✅ | auto-build |
| `M365/SharePoint-OneDrive/_AGENT.md` (backfilled — 1 new overview clause, 3 new Folder-contents rows, 4 new Common-entry-point bullets) | ✅ | auto-build |
| `Windows/_AGENT.md` (backfilled — 1 new overview bullet, 2 new Folder-contents rows, 1 new Common-entry-point bullet) | ✅ | auto-build |
| `AGENT_INDEX.md` (2 new rows: one inserted after the existing Agents-in-SharePoint row, one after the existing Administrator Protection row) | ✅ | auto-build |

_2026-09-03 (run 231, scheduled task "ezadmin-night-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `f4fbe71` (run 230, AD FS DKM Container ACL Hardening / CVE-2026-56155) with no concurrent-run conflicts; file count 1,211. Also inspected the mounted working tree's own `git status`, which continues to show the large modified/untracked-file FUSE-staleness pattern first flagged in run 188 — a `diff -rq` between the mount and the fresh clone confirmed the mount is simply lagging on already-pushed run history (many files present only in the fresh clone, e.g. `DKMACLHardening-A/B.md`, `CloudSyncDeviceSync-A/B.md`, the full `Security/Purview/` DLP-topic backlog, `AdministratorProtection-A/B.md`, `IntelMacRetirement-A/B.md`), with zero genuinely new local-only work found in the mount — consistent with every run since 188. Per standing practice, authored all new work directly against the up-to-date `/tmp` scratch clone rather than the stale mount. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line and narrative-prose mentions of that phrase match the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Microsoft Entra, Microsoft Defender for Endpoint, Microsoft Purview, Windows Server/IT Pro Blog, and SharePoint/OneDrive sources dated August-September 2026, cross-checking every surfaced candidate against the repo via grep before investigating further: the Intune single device page, STIG audit baseline, Multi Admin Approval's Graph-API-automation extension, the Security Administrator role expansion, Entra Tenant Governance GA, the MemberOf rule-operator deprecation, Automatic Endpoint Isolation, AI Agent Runtime Protection, Selective Response Actions, and Microsoft 365 Backup's Full Workload Backup (already covered in `M365-Backup-A.md`'s Full Workload Backup section) were all already confirmed covered by prior runs via grep. Two genuinely new, confirmed-uncovered candidates were built instead, both live-fetched from primary sources before writing anything. First, **Next-Gen Sharing / Hero Link** for SharePoint and OneDrive (Message Center **MC1454378**, staged worldwide rollout late August-late October 2026) — confirmed via repo-wide grep as a complete gap distinct from the existing `Permissions-A/B.md` (the underlying, unchanged permission-inheritance model this topic sits alongside). As of this writing Microsoft has not published a dedicated Learn conceptual article for this feature; live-fetched the M365 Admin community mirror of the full Message Center post text (`m365admin.handsontek.net`) directly rather than relying on search-summary snippets, and flagged this source-confidence gap explicitly in both runbooks' headers per this repo's standing practice for early-rollout, Learn-documentation-pending topics. Built around the single highest-value architectural fact: every file/folder now gets exactly one primary "hero link," and changing its audience **updates the existing link in place** rather than minting a new one — the defining behavioral break from the legacy multi-link model, eliminating an entire class of "I fixed the sharing but they still have the old broken link" ticket by construction. Also built around the deliberately conservative default (`DefaultMainLinkScope = OnlyPeopleAdded`, granting zero access by itself), the coexistence of pre-rollout legacy links under a separate "Other links" section with independently-governed settings, the per-site/per-OneDrive-only `DefaultMainLinkScope` control surface with no tenant-wide equivalent (a genuine MSP-at-scale operational gap), and — the single highest-stakes fact in this topic — the documented, current scope gap where tenant-wide link-expiration policies continue to govern legacy links only, not hero links, a real and currently-unaddressed compliance exposure for any client with contractual/regulatory expiration requirements. Built `Get-HeroLinkSharingAudit.ps1` as a read-only sweep of `DefaultMainLinkScope` across site collections (optionally OneDrive via `-IncludeOneDrive`), explicit in its own header and console output that no supported API exists to enumerate an individual file's current (possibly since-modified) hero-link audience or its legacy "Other links" entries — only the site/OneDrive-level default is queryable. Second, **Post-Quantum Hybrid TLS (ML-KEM)** in Windows — confirmed via repo-wide grep as a complete gap despite this repo's existing AD FS/certificate/Schannel-adjacent coverage. Live-fetched the primary Microsoft Learn "TLS Supported Groups in Windows 11 versions 24H2, Windows Server 2025, and later" Win32 reference page (`ms.date` 2026-05-20, `updated_at` 2026-05-29) directly, confirming the exact three Supported Group strings, their FIPS-mode availability (`No` for all three), and the GA minimum builds, plus the companion `manage-tls` Windows Server Learn page for the pre-existing ECC Curve Order GPO/PowerShell-cmdlet configuration mechanism this feature extends rather than replaces. Built around the single highest-value architectural fact: hybrid negotiation is **bilateral and fails silently to classical** if either peer lacks a matching enabled group — there is no error, warning, or client-visible indication, making "enabled on the server, nothing changed" the single most common and easiest-to-misdiagnose ticket pattern for this topic. Also built around the well-documented, cross-vendor/cross-platform middlebox compatibility risk from the enlarged ClientHello (~300 bytes classical → ~1,200-1,500+ bytes hybrid, corroborated against independent technical sources beyond Microsoft's own documentation given this is a widely-reported issue affecting multiple TLS implementations, not a Windows-specific quirk), Microsoft's own explicit warning that direct registry editing of default TLS priority order is unsupported (Group Policy/TLS cmdlets are the only supported paths even though both are registry-backed under the hood), and the FIPS-mode/hybrid-PQC current mutual exclusivity as a genuine compliance-relevant limitation worth surfacing proactively to regulated clients. Built `Get-PQCTlsReadinessAudit.ps1` as a read-only, local-or-remote (`-ComputerName`, PowerShell remoting) readiness script — build/hotfix eligibility, currently enabled hybrid groups via `Get-TlsEccCurve`, GPO-managed-curve-order detection via `gpresult`, and FIPS mode state — explicit in its own header and console output that it cannot confirm the actually-negotiated group for any live connection (requires a packet capture) nor assess peer/remote-endpoint support, since hybrid PQC readiness is fundamentally bilateral and this script audits only the local/target machine's own configuration state. Backfilled `M365/SharePoint-OneDrive/_AGENT.md` (1 new overview clause, 3 new Folder-contents rows, 4 new Common-entry-point bullets) and `Windows/_AGENT.md` (1 new overview bullet, 2 new Folder-contents rows, 1 new Common-entry-point bullet), and inserted 2 new `AGENT_INDEX.md` rows via targeted Python find-and-insert (one after the existing Agents-in-SharePoint row, one after the existing Administrator Protection row), matching runs 190-230's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (18/18 in HeroLinkSharing-B.md, 16/16 in HeroLinkSharing-A.md, 20/20 in PostQuantumTLS-B.md, 22/22 in PostQuantumTLS-A.md — all even) and `<details>`/`</details>` tag-pair balance verified (7/7, 4/4, 6/6, 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass (25/25 braces, 44/44 parens, 10/10 brackets for the SharePoint script; 45/45 braces, 61/61 parens, 12/12 brackets for the Windows script — both fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up — the persistent large mount-vs-origin lag flagged since run 229 should continue to be surfaced to the user if it hasn't self-corrected. Both topics built this run carry above-average source-confidence caveats worth revisiting: the Hero Link topic has no Microsoft Learn conceptual article yet (sourced from a Message Center post mirror) and should be re-verified once one is published, especially the "no tenant-wide DefaultMainLinkScope" and "expiration policies don't govern hero links" claims which are exactly the kind of detail early rollout documentation sometimes revises; the Post-Quantum TLS topic's GPO-backed registry value name/path is inferred from the long-established (but not newly, explicitly re-documented for the ML-KEM groups specifically) ECC Curve Order mechanism and was deliberately not asserted as a literal registry path in either runbook for this reason. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._

## New Topics — Exchange Online Cross-Tenant Calendar Sharing (EWS to XTAP) + Purview Endpoint DLP Just-in-Time Protection (run 232)
| File | Status | Assigned |
|------|--------|----------|
| `M365/Exchange/CrossTenantCalendarSharing-B.md` | ✅ | auto-build |
| `M365/Exchange/CrossTenantCalendarSharing-A.md` | ✅ | auto-build |
| `M365/Exchange/Scripts/Get-CrossTenantSharingMigrationAudit.ps1` | ✅ | auto-build |
| `Security/Purview/EndpointDLP-JIT-B.md` | ✅ | auto-build |
| `Security/Purview/EndpointDLP-JIT-A.md` | ✅ | auto-build |
| `Security/Purview/Scripts/Get-EndpointDLPJITReadiness.ps1` | ✅ | auto-build |
| `M365/Exchange/_AGENT.md` (backfilled — 1 new overview bullet, 3 new Folder-contents rows, 3 new Common-entry-point bullets) | ✅ | auto-build |
| `Security/Purview/_AGENT.md` (backfilled — 1 new overview sentence, 3 new Folder-contents rows, 3 new Common-entry-point rows) | ✅ | auto-build |
| `AGENT_INDEX.md` (2 new rows inserted directly after the existing Controlled Configuration row) | ✅ | auto-build |

_2026-09-03 (run 232, scheduled task "ezadmin-night-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `0bc6e37` (run 231, SharePoint/OneDrive Hero Link Sharing + Windows Post-Quantum Hybrid TLS) with no concurrent-run conflicts, re-confirmed via a second `git fetch` immediately before writing this run's own files. Also inspected the mounted working tree's own `git status`, which continues to show the large modified/untracked-file FUSE-staleness pattern first flagged in run 188, with the mount's local HEAD still far behind `origin/master` (`f374c6e`/run 169) — per standing practice, did not attempt to reconcile the mount and authored all new work directly against the up-to-date `/tmp` scratch clone. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Microsoft Entra, Exchange Online, Microsoft Purview, and macOS/Jamf sources dated August-September 2026, cross-checking every surfaced candidate against the repo via grep before investigating further: the Intune new single device page, Remote Help unattended sign-in, Platform SSO/Secure Enclave, DDM/legacy macOS update policy retirement, Entra Tenant Governance GA, and the Purview DLP SLA Dashboard were all already confirmed covered by prior runs. Two genuinely new, confirmed-uncovered, and unusually time-sensitive candidates were built instead, both live-fetched from primary Microsoft Learn sources before writing anything. First, **Exchange Online Cross-Tenant Calendar Sharing — EWS to Microsoft 365 Cross-Tenant Access Policy (XTAP) migration** — confirmed via repo-wide grep as a complete gap despite this repo's existing, extensive `M365/Exchange/` coverage and the unrelated `EntraID/Troubleshooting/CrossTenant-B.md` (B2B guest collaboration, not this scenario). Live-fetched the full official Microsoft Learn migration guide (`migrate-to-m365-xtap`, `ms.date` 2026-08-25, `updated_at` 2026-09-01, 3,220 words) directly rather than relying on search-summary snippets, given the topic's precise cmdlet/capability-name requirements. Built around the single highest-value, genuinely urgent fact: Exchange Web Services deprecation in Exchange Online **begins October 1, 2026** — under a month from this run's own date — and three distinct legacy sharing mechanisms (Organization Relationship, Availability Address Space with `OrgWideFBToken`, Sharing Policy) all depend on it for tenant-to-tenant Free/Busy, MailTips, and Calendar Sharing. Also built around the architectural shift from an object-per-relationship legacy model to a capability-per-partner XTAP model requiring a prerequisite Entra `m365CollaborationInbound` trust relationship per partner Tenant ID; the inbound-only, non-reciprocal nature of every XTAP capability (both organizations must independently configure their own side, with no cmdlet able to confirm the partner's own state); and — identified as the single most common post-migration support call — legacy configuration's unconditional precedence over XTAP while still enabled, meaning a fully-built XTAP configuration with zero observable effect almost always means the old object was never disabled. Built `Get-CrossTenantSharingMigrationAudit.ps1` as a read-only inventory/readiness script covering all three legacy mechanisms, mailbox blast-radius sizing via `Get-EXOMailbox -Properties SharingPolicy`, and a best-effort Entra Cross-Tenant Access Policy partner-trust/capability check via Graph beta — explicit in its own header that it cannot confirm any partner organization's own inbound configuration, which always requires direct admin-to-admin coordination. Second, **Microsoft Purview Endpoint DLP Just-in-Time (JIT) Protection** — confirmed via repo-wide grep as a complete, somewhat surprising gap given this repo's otherwise mature Endpoint DLP coverage (`DLP-Policy-A/B.md`, `EndpointDLPExcludedFolders-A/B.md`); the existing `TimeBoxedRoleGroups-A/B.md` "JIT" hits were confirmed to be an unrelated Purview role-group-expiration feature, not this one. Live-fetched both primary Microsoft Learn pages (`endpoint-dlp-get-started-jit`, `ms.date` 2026-06-26; `endpoint-dlp-learn-about-jit`, `ms.date` 2026-06-22) directly. Built around the single highest-value architectural fact: JIT has **no independent detection logic** of its own — it is purely a timing-gap enforcement layer that stands in for an already-configured, existing Block/Block-with-override Endpoint DLP policy while that policy's real classification is still in progress, meaning a tenant with JIT enabled but no Block-capable policy will correctly show zero JIT activity. Also built around the two distinct, easily-conflated timing tracks (resume-capable activities with documented 3s/2s/5s/30s evaluation windows and auto-resume, versus non-resume activities requiring manual retry); the JIT-specific file-path/app/extension exclusion settings being narrower than and independent from the general Endpoint DLP "File path exclusions for Windows" setting (a likely source of "I excluded it and it's still being evaluated" tickets); and clipboard copy being JIT-Audit-by-default unless the productivity-impacting "Control copying to clipboard" setting is explicitly enabled. Also investigated and explicitly flagged, rather than silently presented as settled, the newer **JIT Audit user/group scoping** capability (Microsoft 365 Roadmap ID 562991) — its own Roadmap listing states General Availability, July CY2026, while other secondary trade-press coverage reports a September CY2026 GA target, and no dedicated Learn conceptual page exists for it as of this writing; both runbooks treat its precise interaction with the pre-existing general JIT scope setting as an open, unconfirmed question rather than guessing. Built `Get-EndpointDLPJITReadiness.ps1` as a read-only script checking JIT's own prerequisite (an active Block-capable Endpoint DLP rule via `Get-DlpComplianceRule`) plus a local-or-remote (`-ComputerName`) antimalware client version check against both documented JIT version floors (`4.18.23080` minimum, `4.18.25080` for improved UX) via `Get-MpComputerStatus` — explicit in its own header that it cannot read JIT's own enable/scope/fallback-action/fine-tuning settings, which exist only in the Purview portal with no documented cmdlet surface, nor perform the fleet-wide Advanced Hunting antimalware-version sweep documented inline in both runbooks (that requires the Defender/Security portal). Backfilled `M365/Exchange/_AGENT.md` (1 new overview bullet inserted after the existing Cloud-managed remote mailboxes bullet, 3 new Folder-contents rows after the CloudManagedMailboxAudit script row, 3 new Common-entry-point bullets after the SOA-rollout-readiness entry) and `Security/Purview/_AGENT.md` (1 new sentence appended to the existing overview paragraph, 3 new Folder-contents rows after the NonMicrosoftAppsDLP script row, 3 new Common-entry-point rows after the location-mixing entry), and inserted 2 new `AGENT_INDEX.md` rows directly after the existing Controlled Configuration row via a targeted Python find-and-insert, matching runs 190-231's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (6/6 in CrossTenantCalendarSharing-B.md, 4/4 in CrossTenantCalendarSharing-A.md, 10/10 in EndpointDLP-JIT-B.md, 4/4 in EndpointDLP-JIT-A.md — all even) and `<details>`/`</details>` tag-pair balance verified (6/6, 3/3, 7/7, 3/3 respectively). Script brace/paren/bracket balance verified via a Python counting pass (29/29 braces, 49/49 parens, 6/6 brackets for the Exchange script; 26/26 braces, 38/38 parens, 10/10 brackets for the Purview script — both fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up — the persistent large mount-vs-origin lag flagged since run 229 should continue to be surfaced to the user if it hasn't self-corrected. The Cross-Tenant Calendar Sharing topic carries a live, hard deadline — if a future run's "today" date is on or after October 1, 2026, revisit both runbooks to confirm the EWS deprecation has actually reached the relevant API surface and update "begins October 1, 2026" framing accordingly. The JIT Audit user/group scoping capability's disputed GA date (July vs. September CY2026) and undocumented interaction with the base JIT scope setting should be re-verified once Microsoft publishes a primary Learn page or the discrepancy resolves. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._


---

## New Topic — Apple ADE Enrollment Policies / Settings Catalog Migration (run 233)
| File | Status | Assigned |
|------|--------|---------|
| `macOS/Troubleshooting/ADEEnrollmentPolicies-B.md` | ✅ | auto-build |
| `macOS/Troubleshooting/ADEEnrollmentPolicies-A.md` | ✅ | auto-build |
| `macOS/Scripts/Set-AppleADEDevicesToEnrollmentPolicy.ps1` | ✅ | auto-build |

_2026-09-03 (run 233, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `4e8f646` (run 232, Exchange Online Cross-Tenant Calendar Sharing + Purview Endpoint DLP Just-in-Time Protection) with no concurrent-run conflicts; file count 1,223. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line and narrative-prose mentions match the grep) — Expansion Rules mode. Attempted to open the primary Microsoft Learn/Tech Community source for the new Apple ADE enrollment-policies experience directly but the built-in browser could not reach techcommunity.microsoft.com in this environment; fell back to a `WebSearch`/`web_fetch` sweep across Intune, Entra, Defender for Endpoint, Purview, Windows Autopatch, and macOS/Apple sources dated August-September 2026, cross-checking every surfaced candidate against the repo via grep before investigating further: the STIG audit baseline, Multi Admin Approval's Graph-API-automation extension, Entra Tenant Governance GA, the Security Administrator role expansion, the memberOf operator deprecation, Remote Help unattended sign-in, Copilot retention-policy recommendations, the Windows Autopatch quality-update policy rollout, and the macOS 15 minimum version Plan for Change were all already confirmed covered by prior runs via grep. Microsoft Defender Vulnerability Management's "revised exposure scoring incorporating exploit prediction data and asset criticality" was investigated as a possible new preview feature but found, on inspection of the existing `DefenderVulnMgmt-A.md`, to already describe the same active-exploitation-multiplier/asset-criticality Exposure Score mechanics — judged not a genuine gap rather than built speculatively. One genuinely new, confirmed-uncovered candidate was built instead: **Apple ADE Enrollment Policies (Settings Catalog migration)**, live-fetched from a hands-on community migration walkthrough (sastu-insights.com, dated 2026-07-10, with inline screenshots and a working PowerShell migration script) and a second independent What's-New digest (bighatgroup.com, dated 2026-06-19, covering the same feature's GA announcement) after `web_fetch` against the primary Tech Community post returned only a JS-rendered shell with no body text — no dedicated Microsoft Learn conceptual page exists for this migration as of this writing, a source-confidence gap flagged explicitly in both runbooks' headers per this repo's standing practice, rather than presented as officially settled. Confirmed via repo-wide grep as a complete gap distinct from the existing `ADE-Enrollment-A.md`/`-B.md` (token/push-cert/general enrollment mechanics, which this new topic assumes are already healthy and does not duplicate). Built around the single highest-value architectural fact: the new Enrollment policies blade is not a UI reskin but a full object-model migration onto Settings Catalog (`deviceManagement/configurationPolicies`, `technologies has 'enrollment'`) from the classic `enrollmentProfiles` object type, with every new-style policy mirrored back into the classic collection for compatibility but never populating `isDefault` on the mirror — a detail that silently breaks any existing automation trusting that field alone once a tenant migrates its default. Also built around the flagship new capability, enrollment-time Entra static-group assignment (GA, previously Windows Autopilot/Android Enterprise-only), and its single most support-call-generating failure mode: the requirement that the Intune Provisioning Client service principal (AppId `f1346770-5b25-470b-88bd-d5744ab7952c`) hold group-owner rights, with zero error surfaced anywhere in the portal, Graph responses, or device enrollment logs when that's missing; and the "default is forward-only" gotcha — setting a new policy as default only governs devices syncing in afterward, leaving every already-imported device on its prior assignment until explicitly reassigned, which the portal's device list (no select-all control) makes impractical to do by hand at scale. Built `Set-AppleADEDevicesToEnrollmentPolicy.ps1` as a bulk Graph-based reassignment script mirroring the community source's own tested approach (`updateDeviceProfileAssignment`, serial-number batches, `-WhatIf`/`-SerialNumber`-safe staged rollout support, and the same isDefault-fallback heuristic against `configurationPolicies` `isAssigned eq false` documented in both runbooks) — explicit in its own header that the heuristic can misfire if a tenant carries more than one unassigned same-platform ADE policy on one token, and that reassignment takes effect only at a device's next enrollment, never retroactively. Backfilled `macOS/_AGENT.md` (1 new overview-paragraph bullet inserted after the existing Intel Mac Retirement clause, 1 new Common-entry-point bullet inserted after the existing ADE-Enrollment-B.md entry) and inserted 1 new `AGENT_INDEX.md` row directly after the existing Intel Mac Retirement row, via targeted Python find-and-insert, matching runs 190-232's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (10/10 in ADEEnrollmentPolicies-B.md, 6/6 in ADEEnrollmentPolicies-A.md — both even) and `<details>`/`</details>` tag-pair balance verified (7/7, 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass (40/40 braces, 73/73 parens, 30/30 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up — prior runs (188 onward) have repeatedly found the mounted working tree far behind `origin/master`; this run did not re-inspect mount state directly (worked only against the fresh scratch clone) so that lag should be re-confirmed rather than assumed resolved. The primary Tech Community source for this run's topic could not be fetched directly (JS-rendered, blocked via both `web_fetch` and the built-in browser) — if a future run has working access to techcommunity.microsoft.com, cross-check both new runbooks' claims (especially the exact Graph `creationSource`/template-ID formats and the "no confirmed retirement date" framing for classic profiles) against the official post rather than relying solely on the two independent secondary sources used here. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._

## New Topics — WMIC Removal from Windows + Unified App & Agent Management (Teams/M365 Admin Centers) (run 234)
| File | Status | Assigned |
|------|--------|---------|
| `Windows/Troubleshooting/WMICRemoval-B.md` | ✅ | auto-build |
| `Windows/Troubleshooting/WMICRemoval-A.md` | ✅ | auto-build |
| `Windows/Scripts/Get-WMICUsageAudit.ps1` | ✅ | auto-build |
| `M365/Teams/UnifiedAppAgentManagement-B.md` | ✅ | auto-build |
| `M365/Teams/UnifiedAppAgentManagement-A.md` | ✅ | auto-build |
| `M365/Teams/PreMeetingConsent-A.md` | ✅ | auto-build |
| `M365/Teams/PreMeetingConsent-B.md` | ✅ | auto-build |
| `M365/Teams/ExternalMessagingLimits-B.md` | ✅ | auto-build |
| `M365/Teams/Scripts/Get-UnifiedAppManagementAudit.ps1` | ✅ | auto-build |

_2026-09-03 (run 234, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `3c8d1ea` (run 233, Apple ADE Enrollment Policies / Settings Catalog migration) with no concurrent-run conflicts; file count 1,254. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line and narrative-prose mentions match the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Intune, Microsoft Defender for Endpoint, Microsoft Purview, Microsoft Teams, and Windows 11 IT-pro sources dated August-September 2026, cross-checking every surfaced candidate against the repo via grep before investigating further: Windows Autopilot device preparation/device association GA, Defender's predictive-shielding GPO/Safeboot hardening response actions, the Purview DLP SLA Dashboard, DLP Alert Auto-Resolution and Tags, and DLP Network Filtering via Global Secure Access were all already confirmed covered by prior runs via grep. Two genuinely new, confirmed-uncovered candidates were built instead, both live-fetched from primary Microsoft sources before writing anything. First, **WMIC Removal from Windows** — confirmed via repo-wide grep as a complete gap despite this repo's extensive existing Windows/Troubleshooting coverage. Live-fetched the official Microsoft Support KB directly (KB5067470, "Windows Management Instrumentation Command-line (WMIC) removal from Windows," originally published 2025-09-12, last revised 2026-02-13 — an unusually well-documented, Microsoft-dated primary source, high confidence). Built around the single highest-value architectural fact this repo's Learning Pointers convention exists to surface: WMIC (the thin `wmic.exe` CLI wrapper) and WMI (the underlying management infrastructure) are architecturally distinct, and only the former is being removed — a distinction most tickets on this topic get wrong initially. Also built around the six-step deprecation timeline culminating in the 2026 feature update's **permanent** removal with no Feature on Demand restore path (a qualitative change from every prior, FoD-reversible step in the timeline), the near-mechanical WMIC-to-`Get-CimInstance`/`Invoke-CimMethod` syntax translation model (unchanged underlying WMI class/property names), and the documented `powershell -c "<command>"` bridging pattern for batch files that can't be immediately rewritten. Built `Get-WMICUsageAudit.ps1` as a read-only local/remote (`-ComputerName`) discovery script — WMIC presence/capability-state, OS build timeline placement, Winmgmt service/CIM baseline health (to isolate WMIC-removal symptoms from unrelated WMI-repository issues), and a scheduled-task/local-script-directory scan for `wmic` references — explicit in its own header that it deliberately does NOT enumerate domain GPO SYSVOL logon scripts (a separate, explicitly-called-out follow-on review step in the -A.md runbook's Playbook 1, since that touches domain-wide policy content rather than single-machine state). Second, **Unified App & Agent Management** (Teams admin center ↔ Microsoft 365 admin center convergence) — confirmed via repo-wide grep as a complete gap distinct from this repo's existing `M365/Teams/ExternalAccess-A.md`/`-B.md` (federation/guest access, a different access-control dimension) and `Intune/Troubleshooting/EnterpriseAppManagement-A.md`/`-B.md` (Intune-based app deployment, a separate mechanism from this MAC/TAC availability-and-installation governance layer). Live-fetched both the Message Center post directly (MC796790, 27 revisions tracked since 2024-05-29, last updated 2026-09-01) via its public archive mirror and the official Microsoft Learn conceptual page (`uam-tac-mac`, `ms.date` 2026-04-14, `updated_at` 2026-08-18) before writing anything. Built around the two-phase architecture (Phase A/availability, GA via a three-tier Phase-1-auto/Phase-2-wizard/Phase-3-future-automatic-catchup model; Phase B/installation, rolling out late August through mid-September 2026) and the single highest-value fact for triage: installations made through **Teams app setup policies are permanently excluded** from Phase B unification by architectural design, not a temporary rollout gap — explaining the largest share of likely "why isn't this syncing" tickets without deeper investigation. Also built around the hard Global-Administrator-only consent gate on the wizard's final step (distinct from every earlier, Teams-admin-accessible step) and the documented absence of any rollback path once consent is given. Built `Get-UnifiedAppManagementAudit.ps1` as a read-only Teams-PowerShell-based audit — since no dedicated cmdlet or Graph surface exists for the TAC/MAC unification phase state itself, the script instead enumerates Teams app setup policy assignments (the one directly scriptable, adjacent signal that explains the Phase B exclusion for any given app), with an optional app-name filter for ticket-specific yes/no checks — explicit in its own header and console output about what it cannot read (unification phase state, MAC-side Integrated apps/Copilot Agents settings). Backfilled `Windows/_AGENT.md` (1 new overview bullet after the SSO Permission Auto-Accept clause, 2 new Folder-contents rows, 1 new Common-entry-point bullet) and `M365/Teams/_AGENT.md` (1 new overview-paragraph sentence, 3 new Folder-contents rows, 1 new Common-entry-point bullet), and inserted 2 new `AGENT_INDEX.md` rows directly after the existing Apple ADE Enrollment Policies row via a targeted Python find-and-insert, matching runs 190-233's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (20/20 in WMICRemoval-B.md, 26/26 in WMICRemoval-A.md, 6/6 in UnifiedAppAgentManagement-B.md, 12/12 in UnifiedAppAgentManagement-A.md — all even) and `<details>`/`</details>` tag-pair balance verified (5/5, 4/4, 6/6, 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass (54/54 braces, 81/81 parens, 18/18 brackets for the WMIC script; 29/29 braces, 45/45 parens, 9/9 brackets for the Teams script — both fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up. Both topics built this run are unusually well-sourced directly from Microsoft (an official, dated Support KB for WMIC removal; a 27-times-revised Message Center post plus a conceptual Learn page for Unified App & Agent Management) — worth noting as a contrast to several recent runs' reliance on secondary/community sources for topics lacking mature Microsoft Learn coverage. A fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path — Teams' delegated-calling lock-call enhancement and federated-chat/user-reported-security-signal controls (all surfaced this run via WebSearch but judged either too UI-minor or not yet corroborated against a primary source strongly enough to build) are reasonable candidates to revisit if a future run wants deeper Teams admin-center coverage._

## New Topics — Defender for Endpoint Plug-in for WSL + Apple VPP App Management via DDM (run 235)
| File | Status | Assigned |
|------|--------|---------|
| `Security/Defender/MDE-WSL-B.md` | ✅ | auto-build |
| `Security/Defender/MDE-WSL-A.md` | ✅ | auto-build |
| `Security/Defender/Scripts/Get-MDEWSLPluginHealth.ps1` | ✅ | auto-build |
| `macOS/Troubleshooting/VPP-DDM-B.md` | ✅ | auto-build |
| `macOS/Troubleshooting/VPP-DDM-A.md` | ✅ | auto-build |

_2026-09-03 (run 235, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `62ad2b5` (run 234, WMIC Removal from Windows + Unified App & Agent Management) with no concurrent-run conflicts; file count 1,260. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune and Microsoft Defender for Endpoint sources dated September 2026, then live-fetched the official Microsoft Learn "What's new in Microsoft Intune" page and the Defender for Endpoint "New features" release-notes page directly rather than relying on search-summary snippets, cross-checking every surfaced candidate against the repo via grep before investigating further. Of the items surfaced, Intune Support Assistant, corporate-owned Android eSIM device actions (Activate/Remove/wipe-time eSIM handling), the Windows 365 for Agents security baseline, Selective Response Actions, AI Agent Runtime Protection's Codex CLI/GitHub Copilot app extension, and Automatic Endpoint Isolation were all already confirmed covered by prior runs via grep (the Android eSIM device-action cluster specifically confirmed already comprehensively covered in `Intune/Troubleshooting/AndroidESIM-A.md`, including the exact wipe-time-eSIM-removal and multi-ICCID-inventory details this run's search surfaced as if new). Two genuinely new, confirmed-uncovered candidates were built instead, both live-fetched from primary Microsoft Learn sources before writing anything. First, the **Defender for Endpoint plug-in for Windows Subsystem for Linux (WSL) 2** — confirmed via repo-wide grep as a complete gap despite this repo's extensive existing Defender for Endpoint coverage. Live-fetched the official Microsoft Learn page directly (`mde-plugin-wsl`, `ms.date` 2026-07-02, `updated_at` 2026-08-07). Built around the single highest-value architectural fact: the plug-in loads directly into the virtualized WSL2 subsystem itself (not just the Windows host filesystem), extending visibility-only telemetry (file/process/network, timeline, alerts) into WSL2 containers with an architecturally permanent ceiling — no isolate/AV-scan/live-response-command support on the resulting WSL2 device object, since response actions were never implemented for this device type; containment requires isolating the Windows host instead. Also built around the ~30-minute minimum continuous distro uptime before a WSL2 instance registers as a device object at all (the dominant cause of "plug-in isn't working" false-alarm tickets), the permanent WSL1 invisibility gap (plug-in is WSL2-only), the four-field `healthcheck.exe` health surface and its `--extendedProxy`/`--supportBundle` diagnostics, the documented-deprecated `DefenderProxyServer`/`ConnectivityTest` registry keys superseded by `.wslconfig` settings, and the `HostDeviceId` Advanced Hunting attribute required to join a WSL2 device object back to its Windows host (each WSL2 instance registers as its own separate Linux device object, not a sub-property of the host). Built `Get-MDEWSLPluginHealth.ps1` as a local, read-only host-side readiness script — explicit in its own header that it is host-local only (no Graph/tenant-wide surface exists for this plug-in's configuration or health as of this writing), covering host OS eligibility, Sense onboarding status, WSL version/WSL1-vs-WSL2 per-distro state, plug-in install footprint, `healthcheck.exe` output, `.wslconfig` custom-kernel detection, and device-tag/release-ring registry overrides, while explicitly noting it cannot confirm portal-side device registration itself. Second, **Apple VPP App Management via DDM** (Declarative Device Management management type for Volume Purchase Program apps) — confirmed via repo-wide grep as a genuine gap distinct from this repo's existing `VPP-App-Deployment-A/B.md` (classic MDM-model token/license lifecycle, unchanged by this feature), `DDM-A/B.md` (software update DDM enforcement), and `AppSettings-A/B.md` (app configuration/restriction DDM) — three separate declarative-management surfaces under the same Apple DDM umbrella. Live-fetched the official Microsoft Learn reference page directly (`manage-vpp-apple`, `updated_at` 2026-08-27) before writing anything. Built around the single highest-value fact: Management type (MDM or DDM) is a **token-level** choice made at token creation, not a per-app or per-device setting, and DDM-managed VPP apps currently support only **Required** and **Uninstall** assignment intent — there is no DDM equivalent yet for the classic MDM model's **Available** self-service intent, which this run's research judged (and both runbooks state explicitly) as the single fact most likely to drive early support tickets on this feature. Also built around an explicitly flagged, deliberately unreconciled discrepancy between the Intune What's New announcement's stated OS-version floor (iOS/iPadOS 17.2+, macOS 26+) and the general VPP reference page's stated floor (iOS/iPadOS 18+) for the DDM management model — both runbooks direct the reader to verify the current authoritative floor on the live Learn page rather than repeating either number as settled fact, consistent with this repo's standing practice for recently-announced, still-stabilizing features (e.g. `macOS15MinimumVersion-A.md`'s Plan-for-Change framing). Also built around the new per-app **Automatic app updates** toggle (additive to, not inherited from, the pre-existing token-level MDM-era setting) and the unverified-in-current-documentation question of whether an existing MDM token's Management type can be converted to DDM in place, versus the confirmed-safe path of uploading the location token again as a new token entry. No dedicated script was built for this topic — no Graph/PowerShell surface for VPP token Management type or DDM app declaration status has been confirmed as of this writing (per the -A.md Evidence Pack, which documents this gap explicitly rather than fabricating a script scope beyond what's actually confirmed-readable). Backfilled `Security/Defender/_AGENT.md` (1 new overview-paragraph clause, 2 new Folder-contents rows, 3 new Common-entry-point bullets) and `macOS/_AGENT.md` (1 new Folder-contents row, 1 new Common-entry-point bullet), and inserted 2 new `AGENT_INDEX.md` rows directly after the existing Unified App & Agent Management row via a targeted Python find-and-insert, matching runs 190-234's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (18/18 in MDE-WSL-B.md, 8/8 in MDE-WSL-A.md, 14/14 in VPP-DDM-B.md, 8/8 in VPP-DDM-A.md — all even) and `<details>`/`</details>` tag-pair balance verified (9/9, 3/3, 7/7, 3/3 respectively). Script brace/paren/bracket balance verified via a Python counting pass for `Get-MDEWSLPluginHealth.ps1` (40/40 braces, 64/64 parens, 11/11 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up. The VPP-DDM OS-floor discrepancy flagged this run (17.2+/macOS 26+ vs. 18+) is a good candidate to revisit and resolve once Microsoft's own documentation converges on a single stated floor. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path — this run's WebSearch results leaned heavily on candidates already covered by prior runs (Android eSIM, Windows 365 for Agents baseline, Selective Response Actions), a sign this repo's Microsoft-side coverage continues to be very mature and that live-fetching the actual monthly/dated Learn "what's new" tables (rather than relying on WebSearch's own summarized snippets) remains the more reliable way to surface genuinely new gaps._


---

## Security — Purview Priority Cleanup (Exchange)
| File | Status | Assigned |
|------|--------|---------|
| `Security/Purview/PriorityCleanupExchange-B.md` | ✅ | auto-build |
| `Security/Purview/PriorityCleanupExchange-A.md` | ✅ | auto-build |
| `Security/Purview/Scripts/Get-PriorityCleanupExchangeReadiness.ps1` | ✅ | auto-build |


_2026-09-03 (run 236, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `bda7007` (run 235, Defender for Endpoint plug-in for WSL + Apple VPP App Management via DDM) with no concurrent-run conflicts; re-fetched immediately before writing this run's own changes to close the race window, confirming HEAD was still unchanged. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Defender for Endpoint, and Microsoft Purview sources dated September 2026. Of the items surfaced, the Intune STIG audit baseline, Multi Admin Approval's Graph API automation extension, Copilot & AI App Retention Insights, Endpoint DLP for excluded Windows folders, the DLP SLA dashboard, Network Data Security, and Adaptive Scope lifecycle-status controls were all already confirmed covered via repo-wide grep. One genuinely new, confirmed-uncovered candidate was found and built after a deliberate false-start was ruled out: a WebSearch summary described a "new secure workflow to bypass legal holds and retention policies" as if it were an unbuilt September 2026 Purview feature — grep initially returned zero hits, but live-fetching the two primary sources (Message Center archive MC971035/RM473493 via mc.merill.net) revealed this phrasing is Microsoft's own description of **Purview Priority Cleanup**, already comprehensively covered in this repo for the OneDrive/SharePoint workload (`PriorityCleanupHardDelete-A/B.md`, built under Roadmap 558343/MC1261587) — treated as a near-duplicate and not rebuilt. However, that same investigation surfaced a genuine, confirmed gap: priority cleanup's **Exchange mailbox** configuration is tracked under a separate, distinct Roadmap ID (**473493**, "In development," GA targeted November CY2026) and — unlike the SharePoint/OneDrive variant, which still has no dedicated Learn page — has a live, detailed, recently-updated Microsoft Learn conceptual page (`priority-cleanup-exchange`, `ms.date` 2025-12-03, `updated_at` 2026-07-01) that repo-wide grep confirmed was entirely unreferenced. Live-fetched that page in full before writing anything. Built around the single highest-value architectural fact distinguishing it from the already-covered SharePoint/OneDrive sibling: Exchange priority cleanup requires **three mandatory, individually role-staffed approvers every time** (Priority Cleanup admin, Retention manager, eDiscovery admin — each needing a precise role-combination or policy creation fails outright at save time), versus SharePoint/OneDrive's simpler second-priority-cleanup-admin-plus-eDiscovery-admin model — and reproduced Microsoft's own Exchange-vs-SharePoint/OneDrive comparison table verbatim in both runbooks' Command Cheat Sheet / Learning Pointers for quick reference. Also built around the single shared tenant-wide enable/disable toggle governing both workloads together (a "priority cleanup vanished" ticket for either workload should always prompt checking whether someone recently changed the *other* workload's toggle), the KeyQL query model's four explicitly-unsupported eDiscovery-search properties, the unconditional Preservation Lock override for Exchange (vs. SharePoint/OneDrive's delete-only-config-only exception), the non-configurable end-user-visible Outlook "Retention: `<policy>` (-1 days)" banner and its documented eDiscovery-search-and-purge-first workaround for quiet deletions, and the relabel-instead-of-reject approver mechanic. Built `Get-PriorityCleanupExchangeReadiness.ps1` as a read-only readiness script distinct in scope from the existing SharePoint/OneDrive sibling script — auditing all three Exchange-specific approver-role pools independently (Priority Cleanup Admin, Retention Management, eDiscovery admin via the Search And Purge + Hold pairing), unified audit log ingestion state, an optional per-mailbox 10-MB eligibility floor check, and a best-effort audit-log sweep for the two dedicated operation names — explicit in its own header and console output that it cannot read the shared tenant-wide toggle or any existing policy's own configuration, since no confirmed read surface exists for either. Backfilled `Security/Purview/_AGENT.md` (1 new overview-paragraph clause, 3 new Folder-contents rows, 3 new Common-entry-point bullets — and opportunistically fixed one pre-existing malformed table row from an earlier run that was missing its trailing pipe) and inserted 1 new `AGENT_INDEX.md` row directly after the existing PriorityCleanupHardDelete row via a targeted Python find-and-insert, matching runs 190-235's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (28/28 in PriorityCleanupExchange-B.md, 18/18 in PriorityCleanupExchange-A.md — both even) and `<details>`/`</details>` tag-pair balance verified (7/7 and 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass for `Get-PriorityCleanupExchangeReadiness.ps1` (48/48 braces, 72/72 parens, 17/17 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up. This run is a concrete example of why a WebSearch summary phrase should always be traced to its primary source before either building or discarding it as a duplicate — the initial "bypass legal holds" phrasing looked like a duplicate of the already-covered SharePoint/OneDrive feature, but the underlying Message Center/roadmap sources revealed a genuinely separate, uncovered Exchange-specific configuration one layer beneath the surface phrasing. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._

---

## macOS — Custom Compliance Settings (stale-record correction + new coverage)
| File | Status | Assigned |
|------|--------|---------|
| `macOS/Troubleshooting/CustomCompliance-B.md` | ✅ | auto-build |
| `macOS/Troubleshooting/CustomCompliance-A.md` | ✅ | auto-build |
| `macOS/Scripts/Get-CustomComplianceAudit.ps1` | ✅ | auto-build |

_2026-09-03 (run 237, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `e2b35c2` (run 236, Purview Priority Cleanup for Exchange mailboxes) with no concurrent-run conflicts; file count 1,240. Also diffed the mounted working tree against the fresh clone per the standing FUSE-staleness practice and confirmed the mount was lagging by several runs' worth of files (consistent with every run since 188) — new content in this run was authored directly against the mounted tree only for files confirmed NOT to differ from the fresh clone (verified via the diff first), and large/possibly-stale shared files (`AGENT_INDEX.md`, `_BUILD/MANIFEST.md`) were edited directly in the fresh scratch clone instead, left unmirrored to the mount per the established "mount catches up on its own" practice. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Defender for Endpoint, and Microsoft Entra sources dated September 2026. Of the items surfaced, the Entra ID custom-CSS-branding retirement (Oct 2026), Windows 11 26H2 hotpatching/enablement-package rollout, and Windows Autopatch/device-association topics were all already confirmed covered via repo-wide grep. Two candidates were investigated and handled according to what live verification actually showed, rather than trusting search-summary snippets — consistent with this repo's standing discipline. First, a WebSearch summary claimed Intune now offers a macOS-targeted "Disable MAC address randomization" Wi-Fi profile setting; live-fetching the actual primary source (`ref-wifi-settings-apple`, `ms.date` 2026-06-23/`updated_at` 2026-07-01) showed the setting's own documented scope is explicitly **"This setting applies to: iOS 14.0 and newer, iPadOS 14.0 and newer"** in both the Basic and Enterprise profile sections — macOS is not listed at all. This candidate was disqualified and NOT built, a direct example of a search-summary claim being corrected by live verification of the primary source rather than assumed correct. Second, and built instead: **Custom Compliance Settings for macOS**. The existing `Intune/Troubleshooting/CustomCompliance-A.md`/`-B.md` (built pre-2026-09) explicitly stated in multiple places that custom compliance was "Windows-only" and that macOS was unsupported. Live-fetching the current primary sources (`custom-settings`, `ms.date` 2024-05-15/`updated_at` 2026-07-15; `create-custom-script`, `updated_at` 2026-07-15) confirmed macOS is now documented as a fully supported third platform alongside Windows and Linux, using a Bash discovery script + the same JSON-rules engine — this is a genuine stale-record correction, directly serving this project's own stated "rework and remove stale records" mandate, not just a net-new topic. Built around the single highest-value architectural fact distinguishing the macOS model from the Windows one this repo already documents: a macOS discovery script must satisfy a **dual contract** — a Bash exit code (script execution success/failure) evaluated completely separately from the STDOUT JSON (the actual discovered values) — and conflating the two (e.g., `exit 1` for a genuinely-detected non-compliant setting) produces an `Error` state rather than the intended `NonCompliant`, the most likely authoring mistake for anyone porting Windows-model habits to macOS. Also built around macOS-specific script upload options (run as logged-on user vs. system, signature enforcement, notification suppression), the differing run-time/size limits versus Windows and Linux, BOM corruption as a silent, editor-dependent failure mode unique to the UTF-8-without-BOM requirement, the macOS-specific on-device "Check Status" trigger in Company Portal (distinct from the Windows website-sync and Linux Refresh-button triggers already documented elsewhere in this repo), and an explicitly flagged open question: Microsoft's own troubleshooting section for this feature is titled for "Windows and Linux devices" only, with no macOS-specific confirmation of the same numbered error codes (65007-65010) — both new runbooks direct the reader to verify empirically rather than assume 1:1 parity. Corrected the stale claims directly in `Intune/Troubleshooting/CustomCompliance-A.md` (Out of scope bullet) and `-B.md` (Dependency Cascade, Triage table, and Learning Pointers), each now cross-referencing the new macOS-specific files rather than asserting non-support. Also added a short terminology-disambiguation note in the pre-existing `macOS/Troubleshooting/Compliance-Policies-A.md`'s "Custom Attributes vs Compliance Settings" section, clarifying that Microsoft's current official terminology for the richer JSON+discovery-script model is "Custom Compliance Settings" (this file), distinct from the older single-value "Custom Attributes" description already in that file. Built `Get-CustomComplianceAudit.ps1` as a read-only Graph audit — explicit in its own header and console output that Graph does not expose a reliable custom-vs-built-in-only flag on a compliance policy (the policy candidate list is a heuristic requiring manual portal confirmation), and that it cannot validate discovery script content, exit codes, or JSON output itself, since that requires running the script on a real Mac; it instead reports per-device macOS compliance state (Compliant/NonCompliant/Error/Unknown counts) with configurable staleness flagging to help separate agent-health problems from script-logic problems at a fleet level. Backfilled `macOS/_AGENT.md` (1 new overview-paragraph bullet, 2 new Folder-contents rows, 1 new Common-entry-point bullet) and inserted 1 new `AGENT_INDEX.md` row directly after the existing Legacy macOS Update Policy Retirement row via a targeted Python find-and-insert, matching runs 190-236's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (26/26 in CustomCompliance-B.md, 24/24 in CustomCompliance-A.md — both even) and `<details>`/`</details>` tag-pair balance verified (6/6 and 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass for `Get-CustomComplianceAudit.ps1` (32/32 braces, 36/36 parens, 11/11 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up. The disqualified macOS Wi-Fi MAC-randomization candidate is worth a periodic re-check in case Microsoft extends that setting's documented scope to macOS in a future documentation update — do not rebuild it until the primary source itself lists macOS under "This setting applies to." Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path — this run is also a reminder to periodically re-check OTHER existing runbooks in this repo for similar "as of mid-2026, X is unsupported" phrasing that may have since gone stale, not just to hunt for entirely new topics._

## New Topics — Samsung Knox E-FOTA Firmware Update Management + Intune Properties Catalog Registry Key Collection (run 238)
| File | Status | Assigned |
|------|--------|---------|
| `Intune/Troubleshooting/SamsungKnoxEFOTA-B.md` | ✅ | auto-build |
| `Intune/Troubleshooting/SamsungKnoxEFOTA-A.md` | ✅ | auto-build |
| `Intune/Scripts/Get-KnoxEFOTAReadinessAudit.ps1` | ✅ | auto-build |
| `Intune/Troubleshooting/PropertiesCatalog-B.md` | ✅ | auto-build |
| `Intune/Troubleshooting/PropertiesCatalog-A.md` | ✅ | auto-build |
| `Intune/Scripts/Get-PropertiesCatalogRegistryBudgetAudit.ps1` | ✅ | auto-build |


_2026-09-03 (run 238, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `a8eee654` (run 237, macOS Custom Compliance Settings stale-record correction) with no concurrent-run conflicts; file count 1,243. This run's own day-build task appears to have run after night-build already completed run 237 earlier the same day — consistent with the documented two-scheduled-tasks-can-run-concurrently note. Authored all new content directly against the fresh scratch clone rather than the mounted working tree, per the established practice since run 188 of not trusting the mount's `git status`/file currency for large shared files. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. Ran a `WebSearch` "what's new" sweep across Microsoft Intune, Microsoft Entra, Microsoft Purview, and Windows sources dated September 2026. Of the items surfaced, Entra passkeys-by-default and SMS/voice retirement (effective 2026-09-01), Tenant Governance GA, the Security Administrator role expansion, the MemberOf rule-operator retirement, Purview's Non-Microsoft Apps DLP expansion, the DLP SLA Dashboard, Priority Cleanup Hard Delete, and Time-Boxed Role Groups were all already confirmed covered via repo-wide grep. Two genuinely new, confirmed-uncovered candidates were found by live-fetching the actual dated "Week of ..." entries in the official Microsoft Learn "What's new in Microsoft Intune" page directly (`ms.date`/`updated_at` 2026-09-02) rather than relying on WebSearch summary snippets alone, both cross-checked against repo-wide grep as complete gaps before building. First, **Samsung Knox E-FOTA firmware update management** for corporate-owned Android Enterprise devices (Service Release 2608, August 2026) — confirmed via grep as unreferenced anywhere in the repo (only unrelated "CIEM" substring false-positive hits existed). Live-fetched the official Microsoft Learn how-to page directly (`setup-samsung-knox`, `ms.date` 2026-07-24/`updated_at` 2026-07-25) before writing anything. Built around the single highest-value operational fact likely to drive tickets: despite the feature's "no user interaction" marketing for firmware *deployment*, initial device *registration* is NOT unattended — the end user or a provisioning technician must manually open the Knox E-FOTA app on-device and accept terms and conditions, with no remote or API completion path, meaning kiosk/COSU fleets need this built into a staging checklist rather than assumed as zero-touch the way Autopilot/ADE is. Also built around the layered prerequisite chain (separate Samsung-side Knox E-FOTA license independent of Microsoft licensing, a distinct Samsung Knox administrator credential separate from Entra ID used to authorize the connector, two Managed Google Play required apps, an OEMConfig-not-Settings-Catalog profile with three toggles), the hard COBO/COSU/COPE-only platform restriction (device-administrator and BYOD work-profile enrollments are architecturally ineligible, not just unsupported by policy), and the hourly-not-live refresh cadence for all Samsung-sourced status data. Built `Get-KnoxEFOTAReadinessAudit.ps1` as a read-only Intune-side readiness script — explicit in its own header that it cannot query the Samsung Knox Admin Portal or connector directly (no supported Graph surface exists), covering only platform/ownership eligibility and required-app/OEMConfig presence, with the Samsung-sourced registration/campaign state left to the Monitor tab or Knox Admin Portal by design. Second, **Intune Properties Catalog registry key inventory** (Windows device inventory, Service Release 2607-2608) — confirmed via grep as a complete gap despite this repo's extensive existing device-inventory-adjacent coverage. Live-fetched the official Microsoft Learn how-to page directly (`collect-device-properties`, `ms.date` 2026-07-01/`updated_at` 2026-08-12). Built around the three stacked initial-release hard limits Microsoft's own documentation states explicitly: HKLM-only collection (no HKCU or other hive), a 6 KB per-value cap, and a 100-registry-key-per-device cap that is cumulative across ALL properties-catalog profiles targeting a device, not per-profile — a capacity-planning trap for organizations building profiles independently across teams. Also built around the non-overridable, silently-applied sensitive-value detection logic (a flagged value simply "isn't collected," with no admin notification or exception path documented), the category-level-only stop behavior (no way to remove a single property from an active category), the up-to-24-hour initial collection window, and the 28-day data-visibility tail after profile deletion. Noted but deliberately did not center the runbooks on the **Local AI Agent** property category (OpenClaw discovery) that ships alongside Registry in the same feature — cross-referenced as architecturally distinct from the already-covered `Security/Defender/LocalAIAgentDiscovery-A.md`/`-B.md` mechanism rather than assumed to be the same underlying pipeline. Built `Get-PropertiesCatalogRegistryBudgetAudit.ps1` as a read-only planning/triage script — explicit in its own header that Graph does not expose a stable, parseable view of a profile's actual configured registry-key list, so it flags devices targeted by an unusually high count of properties-catalog/registry-named profiles (by name match) as manual-review candidates rather than claiming an authoritative over-cap determination. Backfilled `Intune/_AGENT.md` (2 new overview-paragraph bullets appended after the existing Android Enterprise eSIM lifecycle bullet, 4 new Folder-contents table rows, 3 new Common-entry-point bullets) and inserted 2 new `AGENT_INDEX.md` rows directly after the existing Android Enterprise eSIM Lifecycle Management row via a targeted Python find-and-insert, matching runs 190-237's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (6/6 in SamsungKnoxEFOTA-B.md, 6/6 in SamsungKnoxEFOTA-A.md, 6/6 in PropertiesCatalog-B.md, 6/6 in PropertiesCatalog-A.md — all even) and `<details>`/`</details>` tag-pair balance verified (6/6, 3/3, 5/5, 3/3 respectively — a stray `</details>` initially left over from copy-paste in SamsungKnoxEFOTA-A.md's Dependency Stack section was caught by this exact check and corrected before finalizing). Script brace/paren/bracket balance verified via a Python counting pass (24/24 braces, 36/36 parens, 7/7 brackets for `Get-KnoxEFOTAReadinessAudit.ps1`; 20/20 braces, 30/30 parens, 9/9 brackets for `Get-PropertiesCatalogRegistryBudgetAudit.ps1` — both fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the mount has caught up. This run is another example of live-fetching the dated Microsoft Learn "what's new" table directly surfacing genuinely new, narrowly-scoped candidates (Samsung Knox E-FOTA, Properties Catalog registry inventory) that a generic WebSearch summary sweep alone did not surface — continue preferring the primary dated source over search-summary snippets. Otherwise a fresh `WebSearch` "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive Tech Community and Message Center posts for October 2026 onward) remains the standing recommended path._

## New Topics — Android Play Integrity / Strong Integrity Enforcement + PIM Iteration 2 (Beta) API Retirement (run 239)
| File | Status | Assigned |
|------|--------|---------|
| `Intune/Troubleshooting/AndroidPlayIntegrity-B.md` | ✅ | auto-build |
| `Intune/Troubleshooting/AndroidPlayIntegrity-A.md` | ✅ | auto-build |
| `Intune/Scripts/Get-AndroidPlayIntegrityAudit.ps1` | ✅ | auto-build |
| `EntraID/Troubleshooting/PIMIteration2Retirement-B.md` | ✅ | auto-build |
| `EntraID/Scripts/Get-PIMIteration2APIUsageAudit.ps1` | ✅ | auto-build |


_2026-09-03 (run 239, scheduled task "ezadmin-day-build"): fresh-clone check confirmed HEAD at `a61c89f` (run 238), 1,249 files, queue empty — Expansion Rules mode. WebSearch "what's new" sweep across Intune/Entra found two confirmed repo gaps via primary-source fetch (both cross-checked via grep first): **Android Play Integrity / Strong Integrity** — Intune enforces Google's May-2025-redefined Strong Integrity criteria (hardware attestation + security patch within trailing 12 months, Android 13+) starting 2026-10-31, source [Intune Customer Success blog](https://techcommunity.microsoft.com/blog/intunecustomersuccess/support-tip-changes-to-google-play-strong-integrity-for-android-13-or-above/4435130) + [Android Enterprise compliance settings reference](https://learn.microsoft.com/en-us/intune/device-security/compliance/ref-android-enterprise-settings); and **PIM Iteration 2 (beta) API retirement** — `/beta/privilegedAccess/{aadRoles,azureResources}` stops returning data 2026-10-28 (MC1181281), with Azure-resources migration landing on Azure Resource Manager rather than Graph, source [Message Center archive](https://mc.merill.net/message/MC1181281) + [Graph PIM iteration 2 reference](https://learn.microsoft.com/en-us/graph/api/resources/privilegedidentitymanagement-root?view=graph-rest-beta) + [PIM API concepts](https://learn.microsoft.com/en-us/entra/id-governance/privileged-identity-management/pim-apis). Built B+A+script for the Android topic, B+script for the PIM topic (narrower, ops/migration-focused — no Mode A warranted). Backfilled `Intune/_AGENT.md` and `EntraID/_AGENT.md` (overview bullet, folder-contents rows, common-entry-point bullets each) and `AGENT_INDEX.md` (2 rows). Fence/`<details>`/script brace-paren-bracket balance all verified clean. B-without-A/A-without-B sweep clean aside from the standing documented exception. Per this project's own stated goal of trimming stale/verbose records, this run's own narrative is intentionally kept short rather than matching the multi-paragraph length of runs 190-238 — future runs should default to this shorter format too. **For next run:** re-run the fresh-clone check first; a fresh WebSearch "what's new" sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive, October 2026 onward) remains the standing path — both Oct 28 and Oct 31, 2026 enforcement dates built this run are now imminent, worth a quick recheck for any last-minute Microsoft guidance changes before those dates pass._

---

## New Topics — Intune App Protection Multiple Managed Accounts (MMA) (run 240)
| File | Status | Assigned |
|------|--------|---------|
| `Intune/Troubleshooting/AppMultipleManagedAccounts-B.md` | ✅ | auto-build |
| `Intune/Troubleshooting/AppMultipleManagedAccounts-A.md` | ✅ | auto-build |
| `Intune/Scripts/Get-MMAReadinessAudit.ps1` | ✅ | auto-build |


_2026-09-03 (run 240, scheduled task "ezadmin-day-build"): fresh-clone check confirmed HEAD at `264c861` (run 239, Android Play Integrity + PIM Iteration 2 retirement), 1,254 files, queue empty — Expansion Rules mode. **WebSearch was unavailable for nearly this entire run** (account spend-limit error on every call after the first three), so this run leaned on `mcp__workspace__web_fetch` against URLs already surfaced by those first three searches (an Entra Sept-2026 techcommunity summary and a Defender Sept-2026 summary — both yielded only already-covered candidates per repo-wide grep: Tenant Governance, Security Administrator role expansion, MemberOf retirement, Selective Response Actions, Automatic Endpoint Isolation, AI Agent Runtime Protection, Windows 365 for Agents, single device page, eSIM) and the official Microsoft Learn "What's new in Microsoft Intune" changelog page directly (`ms.date`/`updated_at` 2026-09-02, fetched in full — techcommunity blog URLs returned empty client-rendered shells via `web_fetch`, consistent with those pages being JS-rendered). Scanning the changelog's dated "Week of..." entries against repo-wide grep surfaced one genuinely new, well-documented gap: **App Protection — Multiple Managed Accounts (MMA)** (Outlook iOS/iPadOS support added week of 2026-07-13; Teams was the original app). Live-fetched the full primary conceptual page (`multiple-managed-accounts`, ms.date 2026-05-15/updated_at 2026-07-14) before writing anything — confirmed via repo-wide grep as a complete gap (`Managed-Apps-A/B.md` covers general MAM policy delivery but not this multi-account behavior). Built around the single highest-value distinction the primary source states explicitly: **segmented view** apps (Teams) enforce per-active-account policy, while **mixed view** apps (Outlook's combined inbox/calendar) hard-lock to full restriction — copy/paste, screen capture, screenshots all blocked — regardless of what any individual account's own policy actually permits, even when only one of the accounts sharing the view is managed. Also built around the one documented admin-facing lever, the iOS app-configuration key `IntuneMAMAllowedAccountsOnly` (a disable switch, not an enable switch, delivered via targeted app configuration policies — a different Graph resource from app protection policies, and a common diagnostic dead-end when engineers only check the latter), the fixed account-composition rule (max one MDM+MAM account per device, additional accounts MAM-only, multiple MDM unsupported), and the device-level (not per-account) Mobile Threat Defense threat-signal sharing model — flagged as particularly relevant for MSP technicians holding managed accounts across multiple client tenants on one device. Built `Get-MMAReadinessAudit.ps1` as a read-only Graph audit — flags managed iOS devices with Outlook/Teams below the documented MMA version floors (Outlook 5.2626.0, Teams 8.10.0) via the device `detectedApps` endpoint, and flags `targetedManagedAppConfigurations` policies setting `IntuneMAMAllowedAccountsOnly = true`; explicit in its own header that no Graph API surface reports actual in-app signed-in account counts or live segmented/mixed-view runtime state. Backfilled `Intune/_AGENT.md` (1 new overview bullet after the Android Play Integrity bullet, 2 new Folder-contents rows, 2 new Common-entry-point bullets) and inserted 1 new `AGENT_INDEX.md` row directly before the existing PIM Iteration 2 row via a targeted Python find-and-insert, matching runs 190-239's large-file-safe editing approach. Markdown code-fence balance verified via `grep -c '^```'` (10/10 in AppMultipleManagedAccounts-B.md, 8/8 in AppMultipleManagedAccounts-A.md — both even) and `<details>`/`</details>` tag-pair balance verified (6/6, 4/4 respectively). Script brace/paren/bracket balance verified via a Python counting pass (29/29 braces, 38/38 parens, 12/12 brackets — fully balanced; no `pwsh` available in this environment to full-parse-check). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). A second candidate, Apple's new "Collect enhanced diagnostic logs from supervised Apple devices" device action (Enhanced Logging via AppleCare-issued token, week of 2026-08-25 in the same changelog), was identified as a confirmed repo gap but deliberately **not built this run** — the only available source is a single one-paragraph changelog entry with no dedicated Microsoft Learn conceptual page found, and building a full Mode A/B pair on that thin a source risked fabricating architectural specifics (exact DDM command/CSP naming, OS-version floor) not actually confirmed anywhere. **For next run:** re-run the standing fresh-clone content diff against `origin/master` first. Retry a genuine `WebSearch` sweep (the spend-limit block may have been session/time-window-scoped, not a durable account change — confirm before assuming it's permanent); if still unavailable, `web_fetch` against Microsoft Learn URLs already in a prior fetch's link text remains usable and is what carried this run, but note technommunity/blog-style "what's new" summary posts return empty JS-rendered shells via `web_fetch` — prefer Microsoft Learn `/en-us/<product>/whats-new/` changelog pages, which render as plain server-side HTML. The Apple Enhanced Logging device action remains a good candidate for a focused follow-up once a dedicated primary-source page (Microsoft Learn or Apple Platform Deployment) can be located and fetched. Otherwise the standing broader sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive, October 2026 onward) remains the recommended path._

_2026-09-03 (run 241, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `6d5dad3` (run 240, Intune App Protection Multiple Managed Accounts), 1,257 files, no concurrent night-build push since. Also inspected the mounted connected-folder working tree directly and found it lagging by roughly 24 runs (last locally-visible commit was run 216) — confirmed this against the fresh clone as the now-familiar staleness pattern rather than genuine local-only work, and worked entirely against the up-to-date `/tmp` clone rather than the stale mount to avoid re-deriving two dozen runs' worth of already-pushed content. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. `WebSearch` was available and used for a full "what's new" sweep across Intune, Windows, Purview, Defender for Endpoint, and Microsoft Teams sources dated September 2026. Of the surfaced candidates, the Intune new single-device-page default, Multi Admin Approval's Graph-API-call extension, the STIG audit baseline, Windows Autopatch hotpatch-restart behavior, the Purview DLP SLA Dashboard, Purview Network DLP via Global Secure Access Internet Access, Purview Copilot retention-policy recommendations, Endpoint DLP excluded-folder coverage, and SharePoint/OneDrive hard-delete were all confirmed already covered via repo-wide grep — this repo's Microsoft-side coverage remains very mature. Two genuinely new, confirmed-uncovered candidates were found in Microsoft Teams and built instead, both sourced from live-fetched Message Center archive mirrors since neither has a Microsoft Learn conceptual page yet. First, **Teams Mandatory Pre-Meeting Consent** (Message Center MC1454114, Roadmap ID 561914, posted 2026-08-14) — confirmed via repo-wide grep as a complete gap, and explicitly distinct from the existing, unrelated recording-consent notification already covered in `Meeting-Policies-A/B.md` (both share the word "consent," a deliberate disambiguation point built into both new runbooks' openings). Live-fetched the full Message Center post via its m365admin.handsontek.net mirror before writing anything. Built around the single highest-value architectural fact stated explicitly in the source: **no participant role is exempt, including the meeting organizer** — a deliberate departure from how most Teams meeting controls work and the most likely source of confused tickets once this rolls out. Also built around the one-mandatory-plus-up-to-four-additional-language content model with silent fallback-to-default for an unmatched device language, the audit-log recording of every consent acknowledgment (flagged in the source's own compliance-considerations table as new customer data being stored — a proactive privacy-review point), the feature being disabled by default with no PowerShell or Graph cmdlet surface documented, and the unusually granular three-ring cloud-gated rollout (Targeted Release Sept 2026 → Worldwide/GCC mid-Oct–mid-Nov 2026 → GCC High/DoD mid-Nov–mid-Dec 2026), which both runbooks treat as the first triage question for any "feature is missing" report. No companion script was built — there is no documented cmdlet or Graph surface for this feature to audit against, only a Teams admin center UI. Second, **Teams External Messaging Limits for MOERA-Only Tenants** (Message Center MC1463510, posted 2026-08-29, GA worldwide 15 September 2026) — confirmed via repo-wide grep as a complete gap. Live-fetched the full Message Center post via its pupuweb.com mirror before writing anything. Built as a Mode B-only hotfix runbook (thin, ops-focused source with no admin configuration surface to warrant a full Mode A architectural treatment) around the single highest-value fact: Microsoft has **not published an exact numeric threshold** for this Teams-side limit, explicitly contrasted against the already-covered, precisely-documented Exchange Online sibling limit (100 external recipients/24h) to prevent an engineer from assuming the two features share a threshold or mechanism. Also built around the default-on, non-configurable nature of the protection (the only admin lever is adding a verified custom domain, which removes the tenant from scope entirely), the explicit statement that internal Teams messaging is unaffected, and the admin throttling alert being planned but **not available at initial rollout** per the source's own wording. Backfilled `M365/Teams/_AGENT.md` (1 new overview-paragraph sentence, 3 new Folder-contents rows, 2 new Common-entry-point bullets) and inserted 2 new `AGENT_INDEX.md` rows directly after the existing Intune MMA row via a targeted Python find-and-insert, matching runs 190-240's large-file-safe editing approach for this manifest's size and the similarly large `_AGENT.md`/`AGENT_INDEX.md` files. Markdown code-fence balance verified via `grep -c '^```'` (10/10 in PreMeetingConsent-A.md, 4/4 in PreMeetingConsent-B.md, 12/12 in ExternalMessagingLimits-B.md — all even) and `<details>`/`</details>` tag-pair balance verified (3/3, 6/6, 3/3 respectively). No PowerShell scripts were built this run (neither topic has a documented cmdlet/Graph surface beyond `Get-MgDomain` for confirming MOERA-only scope, which is referenced inline in `ExternalMessagingLimits-B.md` rather than wrapped in a standalone script). Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note) and this run's own deliberate `ExternalMessagingLimits-B.md`-only build. **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the connected-folder mount has caught up — it was observed roughly 24 runs stale this run and should not be trusted for `git status` without a content diff first. Both topics built this run are Message Center-only with no Learn conceptual page yet; re-verify against Teams admin center directly (and the eventual Learn pages, once published) before treating exact navigation paths or the pre-meeting-consent anonymous-attendee decline workflow as settled. Otherwise the standing broader sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive, October 2026 onward) remains the recommended path._

## New Topics — Microsoft 365 Apps for Mac + macOS Kerberos SSO Extension (run 245)
| File | Status | Assigned |
|------|--------|---------|
| `macOS/Troubleshooting/M365AppsMac-B.md` | ✅ | auto-build |
| `macOS/Troubleshooting/M365AppsMac-A.md` | ✅ | auto-build |
| `macOS/Scripts/Get-M365MacAppsHealth.sh` | ✅ | auto-build |
| `macOS/Troubleshooting/KerberosSSO-B.md` | ✅ | auto-build |
| `macOS/Troubleshooting/KerberosSSO-A.md` | ✅ | auto-build |
| `macOS/Scripts/Get-KerberosSSOStatus.sh` | ✅ | auto-build |

_2026-09-24 (run 245, scheduled task "ezadmin-night-build"; renumbered from 244 during rebase — the concurrent day-build run claimed 244 first with the RDS Sept 2026 CU deadlock + Known Issue Rollback topics): fresh uniquely-named `/tmp` clone, `master` HEAD `ab84d62` (run 243), queue empty (only the Status Key legend matches ⬜) — Expansion Rules mode. September 2026 news sweep (Intune, Exchange Online, Entra, Windows release health) surfaced only already-covered items (cross-tenant Free/Busy → CrossTenantCalendarSharing, M365 Backup full-workload, SSPR explicit-registration → PasskeyDefaultAuth/SSPR, Entra Connect 2.5.79, Sept CU/KB5129195 → MachineIdentityIsolation). Switched to the project's macOS mandate and gap-checked the macOS folder: no coverage of Microsoft 365 Apps for Mac (install/activation/MAU) and Kerberos SSO extension explicitly excluded from Platform-SSO runbooks. Built both from live-fetched sources: Learn `add-microsoft-365-macos` (updated 2026-06-22), `update-office-for-mac-using-msupdate` (ms.date 2026-09-17), `deploy-updates-for-office-for-mac`, `deploy-preferences-for-office-for-mac`, `mau-deadline`, Microsoft Support "remove Office license files on a Mac", Entra `device-join-macos-platform-single-sign-on-kerberos-configuration` (updated 2026-06-15), Apple Platform Deployment "Kerberos SSO extension", and the `app-sso(1)` man page. Scripts follow the macOS folder's established `.sh` device-local convention (not `.ps1`), are read-only, export CSV, and pass `bash -n`. Updated `macOS/_AGENT.md` (2 overview bullets, 2 entry points, 4 folder-contents rows) and `AGENT_INDEX.md` (2 rows). Mounted connected folder remains stale with a stuck `.git/index.lock` (per run 243) — committed/pushed from the `/tmp` clone and rsynced the working tree back to the mount._

---

## New Topics — Machine Identity Isolation Domain Trust Failures (Sept 2026 CU) + Defender for Endpoint on Linux Offline Updates (run 243)
| File | Status | Assigned |
|------|--------|---------|
| `Windows/Troubleshooting/MachineIdentityIsolation-B.md` | ✅ | auto-build |
| `Windows/Troubleshooting/MachineIdentityIsolation-A.md` | ✅ | auto-build |
| `Windows/Scripts/Get-MachineIdentityIsolationAudit.ps1` | ✅ | auto-build |
| `Security/Defender/MDELinuxOfflineUpdates-B.md` | ✅ | auto-build |
| `Security/Defender/MDELinuxOfflineUpdates-A.md` | ✅ | auto-build |
| `Security/Defender/Scripts/Get-MDELinuxOfflineUpdateReadiness.ps1` | ✅ | auto-build |

## New Topics — RDS Session Deadlock after September 2026 CU + Known Issue Rollback (KIR) (run 244)
| File | Status | Assigned |
|------|--------|---------|
| `Windows/Troubleshooting/RDSDeadlockSept2026-B.md` | ✅ | auto-build |
| `Windows/Troubleshooting/RDSDeadlockSept2026-A.md` | ✅ | auto-build |
| `Windows/Scripts/Get-RDSDeadlockSept2026Status.ps1` | ✅ | auto-build |
| `Windows/Troubleshooting/KnownIssueRollback-B.md` | ✅ | auto-build |
| `Windows/Troubleshooting/KnownIssueRollback-A.md` | ✅ | auto-build |
| `Windows/Scripts/Get-KnownIssueRollbackStatus.ps1` | ✅ | auto-build |


_2026-09-19 (run 243, scheduled task "ezadmin-night-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh, uniquely-named `/tmp` scratch directory (a concurrent process, almost certainly this run's own day-build counterpart, was already occupying the conventional `/tmp/ezadmin_scratch` path with an in-progress `nobody`-owned clone at session start, so a distinctly-named directory was used to avoid collision), checked out `master`, and confirmed HEAD at `19fbad7` (run 241, Teams Pre-Meeting Consent + External Messaging Limits), 1,362 files. (Renumbered this section from run 242 to run 243 after rebase — the concurrent day-build run independently claimed run 242 for its own Windows Health Attestation Migration topic and pushed first; run numbers are assigned optimistically at write time and can collide under concurrency, resolve by keeping whichever run reached `origin/master` first and renumbering the other during the rebase.) Also found the mounted connected-folder working tree roughly 70+ runs stale (last real commit visible there was run 169, Aug 18) with a permanently stuck `.git/index.lock` from a crashed process — the FUSE-backed mount does not permit deleting or renaming files (confirmed via failed `rm`/`mv` on the lock file, `Operation not permitted`), so the lock cannot be cleared from within a session; all git operations this run and going forward must use a `/tmp` scratch clone, never the mount's own `.git`. Content-diffed the mount against the fresh clone (1 file only in the mount and not upstream — a stray 12-byte placeholder script, `.patch_teams_agent.py`, safe to ignore) and used `rsync` to bring the mount's working-tree files fully current against `origin/master` before starting new work, so the connected folder now reflects reality for the user even though its own `.git` remains stuck. Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches the grep) — Expansion Rules mode. `WebSearch` was available for a full sweep across Intune, Entra, Defender for Endpoint, Purview, and Windows sources dated September 2026. Of the surfaced candidates, Remote Help unattended sessions, the STIG audit baseline, Multi Admin Approval's Graph API extension, the Security Administrator role expansion, Tenant Governance, passkeys-as-default, the MemberOf rule-operator retirement, Non-Microsoft Apps DLP, Microsoft 365 Archive for SharePoint/OneDrive, Network-layer DLP, Endpoint DLP excluded folders, SharePoint/OneDrive hard delete, Administrator Protection, and Autopilot device-association GA were all confirmed already covered via repo-wide grep. Two genuinely new, confirmed-uncovered candidates were found and built. First, **Machine Identity Isolation domain trust failures** — a live, still-unfolding Microsoft-confirmed known issue (first reports mid-September 2026, Microsoft's own workaround published 2026-09-17), sourced via two dated BleepingComputer articles (one quoting Microsoft's release-health statement directly) rather than a Learn conceptual page, since none exists yet for this specific failure mode — confirmed via repo-wide grep as a complete gap distinct from the unrelated ADCS/certificate trust-relationship runbooks already in `Windows/Troubleshooting/`. Built around the single most field-relevant fact: the September CU did not newly enable the feature, it resumed honoring a dormant setting from before `KB5055523` (April 2026) temporarily disabled the whole feature tenant-wide — meaning a device can break in September for a policy decision made many months earlier. Also built around the Windows Server 2025 Domain Functional Level support boundary (most production domains are currently unsupported and should disable the feature outright), the three provisioning paths requiring matched removal (GPO/Intune/registry), and the documented unjoin/rejoin risk if disabling post-enforcement doesn't resolve via `Test-ComputerSecureChannel -Repair`. Built `Get-MachineIdentityIsolationAudit.ps1` as a read-only per-device classifier (Supported-Healthy/Supported-Broken/Unsupported-Enforcing/Unsupported-Broken), explicit in its header that it makes no repair or domain-membership changes. Second, **Defender for Endpoint on Linux — Offline Security Intelligence Updates** — confirmed via repo-wide grep as a complete gap (existing `MDE-WSL-A/B.md` covers a WSL2-specific, architecturally distinct scenario). Live-fetched the full primary Microsoft Learn page directly (`linux-support-offline-security-intelligence-update`, ms.date 2026-09-08/updated_at 2026-09-11) before writing anything. Built around the mirror-server architecture (a plain file host needing no MDE install, reachable to only two Microsoft URLs), the `offlineDefinitionUpdateUrl` release-ring-folder-excludes-arch_*-subfolder path rule (the most common per-architecture misconfiguration), the three equivalent configuration surfaces (Intune/Defender portal/direct managed JSON), and the new default-on engine signature verification (since engine `101.26062.0005`, August 2026) that fails silently and deprecates the older `offlineDefinitionUpdateVerifySig` setting. Built `Get-MDELinuxOfflineUpdateReadiness.ps1` as a Graph-based Intune Settings Catalog policy-configuration audit (flags enabled-but-no-URL and URL-includes-arch_*-subfolder policies); explicit in its header that it cannot read live device-side `mdatp health` output — no stable Graph surface exists for that, matching this repo's established pattern of being explicit about audit-script limitations. Backfilled `Windows/_AGENT.md` and `Security/Defender/_AGENT.md` (1 new overview bullet/sentence, 2 new Folder-contents rows, 1-2 new Common-entry-point bullets each) and inserted 2 new `AGENT_INDEX.md` rows via targeted Python find-and-insert, matching runs 190-241's large-file-safe editing approach. Markdown code-fence balance verified via `grep -c '^```'` (18/18 MachineIdentityIsolation-B.md, 14/14 -A.md, 18/18 MDELinuxOfflineUpdates-B.md, 10/10 -A.md — all even) and `<details>`/`</details>` tag-pair balance verified (5/5, 4/4, 6/6, 4/4 respectively). Script brace/paren/bracket balance verified via Python counting: `Get-MachineIdentityIsolationAudit.ps1` fully balanced (48/48, 64/64, 10/10); `Get-MDELinuxOfflineUpdateReadiness.ps1` showed 27 open/28 close braces on first pass — traced to a single intentional regex character class (`[^,}]`) containing a literal unpaired `}`, not a real code defect; manually confirmed the actual code-block nesting is correctly paired by inspection. Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the one already-documented, intentional exception (`ReadBasicAllScopeChange-B.md`, per run 208's note). **For next run:** re-run the standing fresh-clone content diff against `origin/master` first — the connected-folder mount's own `.git` has a permanently stuck `index.lock` that cannot be cleared from within a session (FUSE mount blocks delete/rename); always work from a `/tmp` scratch clone and use `rsync` (no `--delete`, since deletion isn't permitted into the mount either) to bring the mount's working tree current for the user's benefit after pushing. Use a distinctly-named `/tmp` scratch directory rather than the conventional name if a concurrent day/night-build run might already be using it. Otherwise the standing broader sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive, October 2026 onward) remains the recommended path._

---

## New Topics — Windows Health Attestation Migration to Microsoft Azure Attestation (run 242)
| File | Status | Assigned |
|------|--------|---------|
| `Intune/Troubleshooting/HealthAttestationAzureMigration-B.md` | ✅ | auto-build |
| `Intune/Troubleshooting/HealthAttestationAzureMigration-A.md` | ✅ | auto-build |
| `Intune/Scripts/Get-AzureAttestationEndpointReadiness.ps1` | ✅ | auto-build |

_2026-09-19 (run 242, scheduled task "ezadmin-day-build"): started with the standing fresh-clone check against `origin/master` before touching anything — cloned a fresh `/tmp` scratch directory, checked out `master`, and confirmed HEAD at `19fbad7` (run 241, Teams Mandatory Pre-Meeting Consent + External Messaging Limits), 1,263 files, no concurrent night-build push since. Also inspected the mounted connected-folder working tree directly and found it lagging badly — last locally-visible commit was run 216, roughly 25 runs stale — confirmed against the fresh clone as the same familiar staleness pattern documented since run 183, and worked entirely against the up-to-date `/tmp` clone rather than re-deriving already-pushed content (an untracked `.patch_teams_agent.py` scratch file was also observed sitting in the mount only, presumably a leftover from an earlier interactive edit session — left untouched, not part of any git-tracked content). Manifest queue confirmed empty entering this run (only the Status Key legend's own literal "⬜ Queued" line matches a grep for the symbol) — Expansion Rules mode. `WebSearch` was available and used for a "what's new" sweep across Intune and Entra sources dated September 2026. Of the surfaced candidates — the Intune new single-device-page default (already covered, confirmed via `SingleDevicePage-A/B.md` already present in the up-to-date clone despite being invisible in the stale mount), the Entra built-in Security Administrator role expansion, Entra Tenant Governance (shadow IT tenant discovery), passkeys as the Sept 1 2026 default Entra ID authentication method, and the MemberOf dynamic-group/AU/entitlement-management rule-operator retirement — every one was confirmed already covered via repo-wide grep (`SecurityAdminRoleExpansion-A/B.md`, `TenantGovernance-A/B.md`, `PasskeyDefaultAuth-A/B.md`, `MemberOfRetirement-A/B.md`), reconfirming this repo's Entra/Intune coverage remains very mature. One genuinely new, confirmed-uncovered candidate was found: Microsoft's **Windows Health Attestation migration from Device Health Attestation (DHA) to Microsoft Azure Attestation (MAA)**, announced as two near-duplicate Message Center posts — **MC1473156** ("Plan for Change," Intune service, posted 2026-09-16) and **MC1473600** (Windows service, posted 2026-09-17, identical content) — both live-fetched via their mc.merill.net archive mirrors before writing anything, and cross-referenced against the live Microsoft Learn `intune/fundamentals/endpoints` page (`ms.date` 2026-08-21), which supplied the full consolidated regional endpoint FQDN list and the legacy DHA fallback endpoint (`has.spserv.microsoft.com`). Built around the single most operationally important fact: this is a service-side, automatic cutover (targeted end of Q1 CY2027) with **no admin-facing opt-in/opt-out control** — the only administrator-actionable work is network readiness (allowlisting the tenant-location-specific regional `*.attest.azure.net` endpoints and exempting them from SSL/TLS inspection). Also built around the two permanent, architecture-level exclusions that don't change even after migration completes — Windows 10 devices and GCC High/DoD tenants both remain on legacy DHA indefinitely — and the narrow scope within compliance policy itself (only the Device Health settings: BitLocker, Secure Boot, Code Integrity; every other compliance setting is unaffected). A genuine primary-source rendering limitation was hit and explicitly flagged rather than papered over: the Learn page's North America/Europe/Asia Pacific regional endpoint tabs render client-side, and only the North America tab's endpoint bullets came through on a direct `web_fetch`; the Europe/Asia Pacific endpoint groupings used in both runbooks and the companion script were reconstructed from the page's separate, complete consolidated FQDN list via standard Azure region-code suffix conventions (`neu`/`weu` = Europe, `jpe` = Japan East/Asia Pacific) and explicitly marked as an inferred grouping needing re-verification against the live tabbed table, not asserted as directly confirmed — consistent with this repo's standing "verify search/render snippets against the primary source, flag rather than guess" discipline. A naming-collision check was also run before choosing filenames: this repo already uses the acronym "MAA" for the unrelated Multi Admin Approval feature (`Troubleshooting/MultiAdminApproval-A/B.md`, `Scripts/Get-MAAAccessPolicyAudit.ps1`) — both new runbooks and the script deliberately spell out "Azure Attestation" throughout and never abbreviate it, with an explicit disambiguation note pointing to `MultiAdminApproval-A/B.md`. The companion script (`Get-AzureAttestationEndpointReadiness.ps1`) does fleet Windows-11-build scoping via a heuristic (OS build ≥ 22000) plus live TCP/443 reachability testing against the full regional endpoint set and the legacy DHA endpoint, explicitly declining to guess at Settings-Catalog-based Device-Health-setting policy content (no stable single Graph field summarizes this) or per-tenant rollout status (no documented Graph field exists) — both are surfaced instead as an explicit portal-only checklist, following the same honest-capability-boundary pattern established by `Get-QualityUpdatePolicyAudit.ps1` and `Get-VulnerabilityRemediationAgentReadiness.ps1`. Markdown code-fence balance verified via `grep -c '^```'` (10/10 in `-B.md`, 6/6 in `-A.md` — both even) and `<details>`/`</details>` tag-pair balance verified (3/3, 4/4 respectively); the PowerShell script's brace/paren/bracket counts were verified balanced via a Python script since no `pwsh`/`powershell` binary was available in the sandbox to run a live parse check. Backfilled `Intune/_AGENT.md` (1 new overview-paragraph clause, 2 new Folder-contents rows, 1 new Common-entry-points bullet) and inserted 1 new `AGENT_INDEX.md` row directly after the existing Apple VPP App Management via DDM row via a targeted Python find-and-insert, matching runs 190-241's large-file-safe editing approach for these files' size. Ran the standing B-without-A/A-without-B sweep after this run's own changes: clean aside from the two already-documented, intentional exceptions (`ReadBasicAllScopeChange-B.md` per run 208, `ExternalMessagingLimits-B.md` per run 241) — this run's own build is a full A/B pair, no new exception introduced. **For next run:** re-run the standing fresh-clone content diff against `origin/master` first and do not assume the connected-folder mount has caught up — it was observed roughly 25 runs stale this run. The Europe/Asia Pacific regional endpoint grouping for the new Azure Attestation topic should be re-verified against the live Microsoft Learn tabbed table (or real tenant behavior) once more of it renders on a future fetch, or once the migration's rollout reaches a real tenant that can be observed directly. Otherwise the standing broader sweep (Entra + Intune + Defender + Purview + Exchange + Windows + macOS + Teams + SharePoint/OneDrive, October 2026 onward) remains the recommended path._

---

_2026-09-24 (run 244, scheduled task "ezadmin-day-build"): fresh-clone check against `origin/master` (HEAD `ab84d62`, run 243); connected-folder mount again stale (git log there shows run 169). Queue empty → Expansion Rules. WebSearch sweep of September 2026 changes surfaced the Sept 8 CU RDS session-teardown deadlock (uncovered; OOB fixes shipped Sept 14) and confirmed Entra Tenant Governance / passkey defaults / user-centric access reviews are already covered. Built RDSDeadlockSept2026-A/B + Get-RDSDeadlockSept2026Status.ps1 and, as its evergreen companion, KnownIssueRollback-A/B + Get-KnownIssueRollbackStatus.ps1 (sourced from Microsoft Learn's KIR GPO/Intune procedure). Community-only details (feature flag 3802373433, WDLIB_Close root cause, registry override, 2012 R2 KB5129243) are explicitly labelled as unofficial. Fence/details balance checked; PS bracket balance checked via Python (no pwsh in sandbox). **For next run:** re-verify whether Microsoft ships an OOB for Server 2016 and a fix for the separate USB Audio Class 1.0 Code 10 regression; update the RDS matrix accordingly._
