...
- In progress: 0
- Queued: 0
- Last updated: 2026-07-06 (auto-build, night run 12: manifest queue still empty. Continued run 11's recommendation to close the two remaining Defender script gaps — `Get-MDAStatus.ps1` (MDA has no local client service, so this checks the two things actually visible from an endpoint: the ForwardCloudAppTraffic policy value that governs MDE→MDA Cloud Discovery forwarding, and MDE/Sense health as a prerequisite for signal quality — reachability to security.microsoft.com and portal.cloudappsecurity.com rounds it out; policy config, alerts, Cloud Discovery data, and CA session-policy application are explicitly out of scope since they're portal/sign-in-log-only) and `Get-NetworkProtectionStatus.ps1` (mirrors the Get-CloudProtectionStatus.ps1 pattern closely since both ride on the same MDA/AMRunningMode chain — checks EnableNetworkProtection state, AMRunningMode as the "silently not enforcing" trap called out in NetworkProtection-B.md, WdNisSvc service health, WdFilter driver presence via fltmc, MAPSReporting level, NIS signature freshness, and counts of recent 1125/1126/1127 events). **This closes out Security/Defender script coverage at 9/9 — every Defender topic now has a companion script.** Also picked up the highest-value gap outside Defender: `Intune/Scripts/Get-EnrollmentDiagnostics.ps1` — Enrollment-B.md has 6 fix paths across licensing/scope/restrictions/stale-records/network/HAADJ but had zero companion script despite almost certainly being the highest ticket-volume Intune topic in a real MSP. Collects dsregcmd /status (parsed into structured fields, not just raw text), derives JoinType (HAADJ vs Entra-only vs domain-only vs unjoined) since the correct fix path depends entirely on this, checks the enrollment scheduled task's existence and state, tests MDM endpoint connectivity, and counts recent DeviceManagement-Enterprise-Diagnostic-Provider errors/warnings. Explicitly notes remote collection may fail on devices with enrollment problems (WinRM itself can be broken) and recommends running locally in that case. Checked for stale `.git/HEAD.lock`/`index.lock` per the standing environment note before committing.
...
- **Script coverage gap now confirmed to extend well beyond Windows/macOS** (which runs 5-7 closed out). As of run 9: EntraID has 14 Troubleshooting topics but only 8 scripts (AccessPackages, AppProxy, CAE, DynamicGroups, IdentityProtection, PRT-Issues, PasswordProtection, SSPR, WHfB still have none); Security/Defender has 9 topics but only 4 scripts (ASR-Rules and Tamper-Protection closed this run — CloudProtection, DefenderVulnMgmt, MDA, MDI, NetworkProtection, WDAC-adjacent topics still gapped beyond WDAC itself); Intune has 20 topics but only 9 scripts; M365/Exchange has 8 topics but only 3 scripts; SharePoint-OneDrive has 3 topics but only 1 script; Teams has 4 topics but only 1 script. Run 9 closed the two highest-value Defender gaps (ASR, Tamper Protection) and the highest-value EntraID gap (MFA). Run 10 closed WHfB, SSPR, and PRT-Issues in EntraID — EntraID script coverage is now 11/14. Run 11 closed CloudProtection, MDI, and DefenderVulnMgmt in Defender — Defender script coverage was 7/9. **Run 12 closed MDA and NetworkProtection — Security/Defender script coverage is now complete at 9/9.** Run 12 also closed the Intune/Enrollment gap (now 10/20 — Enrollment was the single highest ticket-volume Intune topic and had zero script). Remaining gaps: EntraID AccessPackages, AppProxy, CAE, DynamicGroups, IdentityProtection, PasswordProtection (6 of 14 topics as of run 12 — **run 13 closed AccessPackages, CAE, and IdentityProtection, bringing EntraID to 11/14; remaining: AppProxy, DynamicGroups, PasswordProtection**); Intune has 19 more topics without scripts (App-Deployment, AppProtection, Autopatch, CoManagement, CustomCompliance, DriverManagement, EPM, FeatureUpdates, Filters, GP-to-CSP, Kiosk, Managed-Apps, Platform-Scripts, Policy-Conflict, Remediations, ScopeTags, WUfB — largest remaining topic-to-script ratio gap in the repo); Exchange (8 topics/3 scripts), SharePoint-OneDrive (3/1), and Teams (4/1) are untouched.
