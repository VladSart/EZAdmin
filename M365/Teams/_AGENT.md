...
- `macOS/Troubleshooting/TeamsMac-B.md` / `-A.md` — Teams **client on Mac** (install, cache, sign-in, screen-share/camera/mic TCC, `CloudType`, audio driver); this folder covers tenant-side policy
...
| `ExternalMessagingLimits-A.md` | External messaging limits deep dive — tenant-scoped (not sender-scoped) trigger, why no threshold is published, comparison with the Exchange 100-external-recipients/24h MOERA cap and the 2024 trial-tenant federation block, guest-vs-federated ambiguity, detection gap (no cmdlet/alert yet), custom-domain adoption and risk-acceptance playbooks |
| `Scripts/Get-MOERAOnlyTenantExposure.ps1` | Read-only MOERA exposure audit — verified custom domain present?, MOERA default domain, optional member users on onmicrosoft.com UPNs, mailboxes with MOERA primary SMTP (Exchange cap exposure), Teams federation config to separate policy blocks from throttling |
...
- "User temporarily can't message an external contact / Teams shows an external-messaging-limited notification" → `ExternalMessagingLimits-B.md` Triage — confirm tenant is MOERA-only via `Get-MgDomain` before assuming this feature is the cause; architecture and cross-workload (Exchange cap) context in `ExternalMessagingLimits-A.md`; onboarding/one-shot audit via `Scripts/Get-MOERAOnlyTenantExposure.ps1 -IncludeUsers -IncludeExchange -IncludeTeams`
