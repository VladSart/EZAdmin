...
- **Azure Front Door's Private Link origins (Premium tier)** — a specific application of this same underlying Private Link mechanism to connect Front Door to an origin privately, referenced in `FrontDoor-A.md` as a likely future topic of its own; the DNS/approval mechanics documented here apply, but Front Door's specific origin-configuration workflow is not covered in this file.
