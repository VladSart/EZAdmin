...
- **MDM supervision is required for silent extension approval:** On supervised devices (enrolled via ADE/Apple Business), an MDM profile can auto-approve System Extensions without any user prompt. On user-enrolled (non-supervised) devices, the user MUST manually approve in System Settings. This is a macOS security design, not a configuration error. See [MDM System Extension payload](https://developer.apple.com/documentation/devicemanagement/systemextensions).
