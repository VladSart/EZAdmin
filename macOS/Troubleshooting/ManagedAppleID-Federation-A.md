...
**The rebrand context.** Apple merged Apple Business Manager, Apple Business Essentials, and Apple Business Connect into a single console called **Apple Business** (business.apple.com). The underlying tenant, domains, users, and devices are unchanged — only the marketing name and unified console changed. This repo's other macOS files (`ABM-Token-Renewal-A.md`, `DeviceMigration-A.md`, etc.) have since been updated to use "Apple Business" too, though "ABM" remains in use as an established shorthand (matching this repo's own `ABM-Token-Renewal` filenames) — same product either way. Expect both "Apple Business" and "Apple Business Manager" in the wild for a while yet, since Apple's own older support articles and third-party material haven't fully caught up.
