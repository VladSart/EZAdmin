...
- **Major upgrades need MDM-supervised devices:** A device must be MDM-supervised (Apple Business enrolled via ADE, not user-enrolled) to receive forced OS upgrades via MDM. User-enrolled Macs can only be nudged — the user must ultimately approve. Check supervision status: `profiles status -type enrollment | grep Supervised`. [Apple MDM Protocol Reference: ScheduleOSUpdate](https://developer.apple.com/documentation/devicemanagement/schedule_an_os_update)
