...
| `Troubleshooting/ADFS/DKMACLHardening-B.md` | Hotfix: CVE-2026-56155 DKM container ACL triage, Event 1132-1136 interpretation, opt-in remediation for WS2016+ and WS2012/2012R2, rollback via saved SDDL |
| `Troubleshooting/ADFS/DKMACLHardening-A.md` | Deep dive: DKM key-wrapping architecture, Golden-SAML-adjacent risk model, phased Audit/Enforcement rollout mechanics, fleet-wide remediation playbooks |
| `Scripts/Get-DKMACLHardeningAudit.ps1` | Read-only audit: DKM container ACL vs. secure baseline, patch-level phase detection, RemediateDkmAcl registry state, recent 1132-1136 events, multi-node support |
...
- "Do we need to do anything about the AD FS DKM/CVE-2026-56155 security update?" → `Troubleshooting/ADFS/DKMACLHardening-B.md` (Triage — deadline context and current phase)
- "AD FS event log is logging 1132/1134/1136 warnings or errors" → `Troubleshooting/ADFS/DKMACLHardening-B.md` (Signal table)
- "Are our AD FS servers exposed to the DKM container ACL vulnerability?" → `Scripts/Get-DKMACLHardeningAudit.ps1` (read-only fleet audit) then `Troubleshooting/ADFS/DKMACLHardening-A.md` for remediation playbooks
