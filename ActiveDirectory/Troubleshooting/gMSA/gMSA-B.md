...
- Windows Server 2025 introduced delegated Managed Service Accounts (dMSA) as an evolution aimed at easing migration off legacy (non-managed) service accounts and reducing Kerberoasting exposure — if the ticket actually names a dMSA (not a gMSA), go to `ActiveDirectory/Troubleshooting/dMSA/dMSA-B.md` instead, since dMSA has its own platform gate, migration state machine, and client-side policy switch not present in this gMSA-focused runbook.
