...
- **New single device page (console UI)** — the redesigned device-details experience in the Intune admin center, becoming the **default and sole** layout for all admins with the **September 2026 (2609) service release**, fully retiring the legacy multi-tab device page. Presentation-layer only — no change to the underlying Graph data model, compliance evaluation, or device actions. Reorganizes content into Essentials/Device action status/Tools and reports/Properties/Device details, and enforces RBAC-driven action visibility more strictly than before (hides, rather than greys out, ungranted actions) — the single biggest source of "a button disappeared" tickets after rollout
- **eSIM cellular profile deployment (Windows Connected PCs)** — bulk activation of embedded SIM data connections via two independent, non-interchangeable methods: CSV activation-code import (public preview, Windows 10/11, one-time-use codes) and the newer, recommended eSIM download server / SM-DP+ FQDN method (Windows 11 only, Settings Catalog policy, EID-based, no individual codes). Both share the same underlying eUICCs CSP and the same hard limitation — Intune's delivery status confirms only its own half of the handshake; actual profile download/activation is an operator-side process with no remote Intune visibility, and there is no supported remote profile-removal action for either method
...
| `Troubleshooting/SingleDevicePage-B.md` / `-A.md` | Hotfix / deep dive: the new single device page becoming the mandatory default with the 2609 release — separating navigation/RBAC-visibility confusion (the vast majority of tickets) from genuine data issues, the legacy-tab-to-new-section mapping, and browser-side rendering triage |
| `Troubleshooting/eSIM-B.md` / `-A.md` | Hotfix / deep dive: eSIM cellular profile bulk deployment — CSV activation-code import vs. eSIM download server (SM-DP+) methods, their differing Windows-version support, the CSV file contract, the hard Intune-visibility boundary at the operator-side activation handshake, and the absence of a remote profile-removal action |
| `Scripts/Get-eSIMDeploymentStatus.ps1` | Read-only Graph audit of eSIM policy/pool delivery status across both deployment methods per device — flags likely OS-version/method mismatches; explicitly cannot confirm on-device profile activation (device-local or operator-side only) |
...
- "Where did the Hardware/Overview tab go" / "admin can't find a device action after an update" / "device page looks completely different" → `Troubleshooting/SingleDevicePage-B.md` Fix 1 (legacy-tab-to-new-section mapping) — confirm it's navigation, not data, via a quick Graph cross-check first
- "Device action button is just gone for this admin" → `Troubleshooting/SingleDevicePage-B.md` Fix 2 — the new page hides (not greys out) actions outside the signed-in admin's RBAC role
- "eSIM activation code won't activate" / "eSIM download server FQDN configured but no profile on device" / "can't remove an eSIM profile remotely" → `Troubleshooting/eSIM-B.md` + `Scripts/Get-eSIMDeploymentStatus.ps1` (identify CSV vs. download-server method FIRST — they fail differently)
