...
- **Vulnerability Remediation Agent (public preview)** — a Security Copilot agent surfaced in the Intune admin center (Agents or Endpoint security node) that correlates Defender Vulnerability Management CVE data against managed Windows devices and produces CVSS/exposure-prioritized, Intune-specific step-by-step remediation guidance. Runs under a dedicated Microsoft Entra agentic identity provisioned at setup — NOT the setup admin's own credentials — with three separately-scoped permission sets (set up/manage, run via the agentic user delegated in the Entra AND Defender admin centers outside the setup wizard, and view results), gated by a Run Readiness Check before Run/Schedule activate. Legacy agents on a human identity have a 90-day, one-way expiration window to transition. Windows Server devices are structurally excluded from CVE counts/exposed-device lists; scope tags are unsupported in preview; and any admin with Intune admin center access can see agent-suggestion data regardless of assigned Intune role/scope — a notable caveat for MSP tenants
- **Android Enterprise eSIM lifecycle (corporate-owned)** — single-device Activate eSIM / Remove eSIM actions and wipe-time eSIM-removal control for COBO/COSU/COPE devices (Intune Service Release 2608, August 2026); BYOD work-profile devices unconditionally unsupported; Activate uniformly requires Android 15+, but Remove has an ownership-type-specific asymmetric floor (COBO/COSU 15+, COPE 17+); no slot-capacity pre-check before Activate, ICCID-must-already-be-in-inventory precondition for Remove — architecturally unrelated to the separate Windows Connected PC eSIM bulk-deployment topic below
- **Samsung Knox E-FOTA firmware update management (Android Enterprise, corporate-owned)** — Intune admin-center integration with Samsung's own Knox E-FOTA service for remote firmware campaign management on COBO/COSU/COPE devices; requires a separate Samsung Knox E-FOTA license, a distinct Samsung Knox administrator credential to authorize the connector, two Managed Google Play apps (Knox E-FOTA, Knox Service Plugin) plus an OEMConfig profile, and — despite "no user interaction" deployment marketing — a manual on-device terms-acceptance step by the end user/technician to complete registration; all status data (registration report, campaign Monitor tab) refreshes hourly from Samsung, never live
- **Properties catalog (Windows device inventory / registry key collection, Service Release 2607-2608)** — no-script mechanism to collect hardware/config/security-signal properties, including HKLM-only registry key values (single value / non-recursive key contents / same value across immediate subkeys), from Windows devices only; per-device caps of 100 registry keys and 6 KB per value, silent non-overridable exclusion of values flagged by sensitive-data detection, category-level-only stop (no single-property removal), and up to 24h initial collection / 28-day post-deletion data retention; also the collection path for the **Local AI Agent** category (OpenClaw discovery via Device Query) — a distinct mechanism from Defender's own local AI agent discovery
- **Android Play Integrity / Strong Integrity (Android Enterprise compliance)** — the Google Play Integrity Verdict compliance/app-protection setting (basic / device / strong integrity tiers) and the October 31, 2026 Intune enforcement of Google's redefined Strong Integrity criteria for Android 13+ devices, which now additionally requires a security patch within the trailing 12 months on top of hardware-backed key attestation; applies to both company-owned and personally-owned work profile device sections independently
- **App Protection — Multiple Managed Accounts (MMA)** — lets a single MMA-integrated app hold more than one Intune-MAM-managed identity at once, each governed by its own app protection policy; currently Microsoft Teams (iOS/iPadOS 8.10.0+, segmented per-account view) and Microsoft Outlook (iOS/iPadOS 5.2626.0+, mixed combined-view — which hard-locks to full restriction regardless of any individual account's own policy); max one MDM+MAM account per device, additional accounts MAM-only; the only admin-facing lever is the iOS app-configuration key `IntuneMAMAllowedAccountsOnly`, a disable switch, not an enable switch
...
| `Troubleshooting/StoreAppsWinGet-B.md` / `-A.md` | Hotfix / deep dive: **Microsoft Store app (new)**, delivered by IME through WinGet (Desktop App Installer, `msstore` source). Covers the IME + engine dependency, the policy matrix (Turn off the Store application does NOT block Intune; RequirePrivateStoreOnly and DesktopAppInstaller EnableAppInstaller/EnableMicrosoftStoreSource can), the System vs User context rules (Entra registered = System only; 0x87D1041C mixed-context detection), update ownership (Store updates UWP, Intune updates Win32 Store apps only while assigned), vendor-hosted Win32 content, 0x8A15xxxx decoding, and migrating off legacy MSfB/Store (legacy) objects |
| `Troubleshooting/Win32AppRelationships-B.md` / `-A.md` + `Scripts/Get-Win32AppRelationshipAudit.ps1` | Hotfix / deep dive: Win32 app **supersedence** (update vs replace, chains collapse to head, 11-node graph limit, no auto-targeting) and **dependencies** (detect vs autoInstall, 100-app graph, no targeting needed), the documented dependency/supersedence conflict and orphan patterns, Available-app auto-update consent loss, Mobile apps: Relate RBAC; tenant-wide Graph topology audit (`mobileApps/{id}/relationships`) |
...
| `Troubleshooting/VulnerabilityRemediationAgent-B.md` / `-A.md` | Hotfix / deep dive: the Vulnerability Remediation Agent (Security Copilot, public preview) — the three-separately-scoped-permission-set model (set up/manage, run via a dedicated Entra agentic identity delegated outside the setup wizard, view results), the Run Readiness Check gate, the one-way 90-day human-to-agentic identity transition deadline, the Windows-client-only/Server-excluded CVE scope, no scope-tag support in preview, and the documented cross-scope agent-suggestion visibility caveat for MSP tenants |
...
| `Troubleshooting/eSIM-B.md` / `-A.md` | Hotfix / deep dive: eSIM cellular profile deployment for Windows Connected PCs — CSV activation-code import vs. eSIM download server (SM-DP+), delivery-status-vs-actual-activation distinction, no remote profile-removal action (backfilled into this table — file predates this entry) |
| `Scripts/Get-eSIMDeploymentStatus.ps1` | Read-only audit of both Windows Connected PC eSIM bulk-deployment methods' Intune-side delivery/assignment status (backfilled into this table — file predates this entry) |
| `Troubleshooting/AndroidESIM-B.md` / `-A.md` | Hotfix / deep dive: Android Enterprise eSIM lifecycle for corporate-owned devices — Activate/Remove single-device actions, ownership-type-asymmetric Android version floors (COPE 17+ vs. COBO/COSU 15+ for Remove), ICCID-inventory precondition, no slot-capacity pre-check on Activate, wipe-time preserve-by-default behavior — architecturally distinct from Windows Connected PC eSIM above |
| `Scripts/Get-AndroidESIMLifecycleAudit.ps1` | Read-only eligibility audit — per-device Activate/Remove eligibility against ownership-type-specific version floors, BYOD exclusion flagging, hardware-inventory-completeness signal |
...
| `Scripts/Get-StoreAppWinGetDiagnostics.ps1` | Read-only device check for Store app (new) failures: IME service/version, Desktop App Installer + winget.exe resolution, cores/arch, join type vs install context, Store/AppInstaller policies from MDM and GPO flagged as blocking or harmless, WinHTTP proxy + Store/WinGet endpoint 443 tests, recent 0x8A15xxxx/0x87D1041C lines from AppWorkload.log, optional -PackageId / -AppNameLike per-user vs provisioned context → CSV |
...
| `Scripts/Get-VulnerabilityRemediationAgentReadiness.ps1` | Graph-based Windows client-vs-server device population scoping (the agent's actual in-scope set) + best-effort DVM-reachability sample; explicit that Security Copilot licensing/SCUs, plugin state, agentic identity type, and Run Readiness Check status are all portal-only and cannot be read via this script |
...
| `Troubleshooting/SamsungKnoxEFOTA-B.md` / `-A.md` | Hotfix / deep dive: Samsung Knox E-FOTA firmware campaign management for corporate-owned Android Enterprise devices — connector/app/OEMConfig chain, manual device-registration step, campaign targeting |
| `Scripts/Get-KnoxEFOTAReadinessAudit.ps1` | Platform eligibility + required-app install status + OEMConfig profile presence audit ahead of/during Knox E-FOTA rollout (cannot query Samsung-side registration/campaign state) |
| `Troubleshooting/PropertiesCatalog-B.md` / `-A.md` | Hotfix / deep dive: properties catalog device inventory / registry key collection — HKLM-only, 100-key/6KB caps, silent sensitive-value filtering, category-level-only stop |
| `Scripts/Get-PropertiesCatalogRegistryBudgetAudit.ps1` | Flags devices targeted by multiple properties-catalog/registry-named profiles as candidates for manual 100-key-cap review (cannot read profile payload content directly) |
| `Troubleshooting/DeploymentPlans-B.md` / `-A.md` | Hotfix / deep dive: Intune Deployments (public preview): ring-based staged rollout of Win32 / Enterprise App Catalog apps and Settings catalog / Endpoint security policies (Windows only). Covers deployment plans (reusable templates, own CRUD permission + scope tags) vs deployments (one payload, immutable rings, payload-borrowed RBAC), cumulative ring-to-payload assignment writing, virtual-group final-ring REPLACE, collision and deleted-group error states, Multi Admin Approval gating of Create/Resume/Cancel/Delete, and non-rollback Cancel |
| `Scripts/Get-IntuneDeploymentReadiness.ps1` | Pre-flight for a Deployments payload: supported type/platform, non-Required app intents, existing All users/All devices assignments, collisions between planned ring groups and existing payload assignments, soft-deleted/missing ring groups (does not read deployment objects; no documented Graph surface in preview) |
| `Troubleshooting/DeviceQuery-B.md` / `-A.md` | Hotfix / deep dive: Advanced Analytics Device query — single-device live KQL (Windows, corporate, Entra/hybrid joined, WNS-mandatory, Managed Devices/Query RBAC, 15/min, 128 KB) vs device query for multiple devices (inventory across Windows/Apple/Android corp, properties catalog required for Windows, 10/min + 1,000/month, ≤3 joins, `Device` entity rules) |
| `Scripts/Get-DeviceQueryReadiness.ps1` | Device-local readiness: join type, MDM enrollment, WpnService, WNS 443, NoCloudApplicationNotification policy, WinHTTP proxy, Device Inventory Agent; optional Graph ownership check |
| `Troubleshooting/AdvancedAnalytics-B.md` / `-A.md` | Hotfix / deep dive: Advanced Analytics report layer — Anomalies (4 fixed statistical models, volume-gated, correlation groups only for Medium/High), Device timeline (replaces App reliability tab, Intune/co-managed only, admin-time-zone timestamps), Battery health (<60 %/60–80 % capacity, <3 h runtime, App impact), Resource performance (CPU >50 %/RAM >75 % spikes, incl. Windows 365 Cloud PCs, not in DoD), custom Device scopes (1 tag, ≥10 devices, 20 active/100 saved); licence auto-enable ≤48 h |
| `Scripts/Get-AdvancedAnalyticsReportAudit.ps1` | Read-only Graph beta audit: AA licence service-plan check, active Medium/High anomalies, battery devices below capacity/runtime thresholds, resource-performance devices below score threshold — CSV per dataset |
| `Troubleshooting/MicrosoftTunnel-B.md` / `-A.md` | Hotfix / deep dive: Microsoft Tunnel Gateway (Intune VPN for iOS/Android) — two-container Linux architecture (mstunnel-agent/mstunnel-server ocserv), supported distro/runtime matrix, ip_forward/ip_tables/tun kernel prereqs, TCP+UDP 443 inbound, no TLS-inspection/auth proxy, Docker/Podman bridge overlap, TLS cert SAN/398-day iOS rule, health-check thresholds, known issues (Offline-but-working → reinstall, /dev/net/tun after reboot, Podman checkup errors), 8-hour verbose Send logs |
| `Scripts/Get-MicrosoftTunnelHealthAudit.ps1` | Read-only Graph beta audit of Tunnel Sites/Servers/Server configurations: unhealthy or stale-check-in servers, empty Sites, missing probe URL, auto-upgrade off / upgrade available, missing DNS, client pool overlapping Docker/Podman default bridges; CSV |
| `Troubleshooting/EdgeManagement-B.md` / `-A.md` | Hotfix / deep dive: Microsoft Edge for Business policy source resolution — Edge management service (M365 admin center) Cloud-type vs Intune-type policies, GPO/MDM platform precedence, cloud device (enrollment token) vs cloud user (group) scope, EdgeManagementEnabled / PolicyOverridesPlatformPolicy / UserPolicyOverridesCloudMachinePolicy switches, 90 min/24 h refresh + restart requirement, primary-profile-only, GDAP not fully supported, not in GCC |
| `Scripts/Get-EdgePolicySourceAudit.ps1` | Local read-only snapshot: Edge version gate, management-service switches in HKLM/HKCU (token redacted), all mandatory/recommended platform policy values incl. list subkeys, MDM PolicyManager Edge-area heuristic, GPO hint, optional single-policy locator; CSV |
| `Troubleshooting/AndroidPlayIntegrity-B.md` / `-A.md` | Hotfix / deep dive: Google Play Integrity Verdict / Strong Integrity compliance evaluation on Android Enterprise — the Oct 31, 2026 enforcement of Google's redefined strong-integrity patch-recency requirement for Android 13+ |
| `Scripts/Get-AndroidPlayIntegrityAudit.ps1` | Fleet-wide audit flagging Android 13+ managed devices whose security patch is already outside (or approaching) the trailing 12-month strong-integrity window |
| `Troubleshooting/AppMultipleManagedAccounts-B.md` / `-A.md` | Hotfix / deep dive: Multiple Managed Accounts (MMA) app protection — segmented (Teams) vs. mixed (Outlook) view enforcement models, the `IntuneMAMAllowedAccountsOnly` app-config disable key, MDM+MAM account-composition limits, and device-level (not per-account) Mobile Threat Defense signal sharing |
| `Scripts/Get-MMAReadinessAudit.ps1` | Read-only audit — flags managed iOS devices with Outlook/Teams below the MMA version floor, and app configuration policies setting `IntuneMAMAllowedAccountsOnly = true` that would silently block MMA |
| `Troubleshooting/HealthAttestationAzureMigration-B.md` / `-A.md` | Hotfix / deep dive: Windows Health Attestation migration from legacy Device Health Attestation (DHA) to Microsoft Azure Attestation — Windows 11-only, non-GCCH/DoD-only scope, network-readiness-only remediation (regional `*.attest.azure.net` endpoint allowlisting + SSL inspection bypass), permanent DHA carve-outs for Windows 10 and GCC High/DoD |
| `Scripts/Get-AzureAttestationEndpointReadiness.ps1` | Fleet Windows-11 scoping heuristic + regional Azure Attestation / legacy DHA endpoint reachability sweep + portal-only evidence checklist (compliance-policy Device Health setting content and per-tenant rollout status have no stable Graph field) |
...
- "Windows 11 device suddenly non-compliant on BitLocker/Secure Boot/Code Integrity, no policy change made" / "compliance broke after a Windows Health Attestation announcement" → `Troubleshooting/HealthAttestationAzureMigration-B.md` (Fix 1 — check regional Azure Attestation endpoint reachability + SSL inspection first; confirm Windows 10 or GCC High/DoD is out of scope via Fix 2)
...
- "Microsoft Store app stuck at Install pending / Failed" / "0x8A15001B" / "0x87D1041C but the app is installed" / "Requirements Not Met in Company Portal" / "we turned off the Store, will Intune Store apps still work?" → `Troubleshooting/StoreAppsWinGet-B.md` + `Scripts/Get-StoreAppWinGetDiagnostics.ps1` (deep dive: `StoreAppsWinGet-A.md`)
- "New app version never replaces the old one" / "both versions installed after supersedence" / "app shows conflict" / "Available app didn't auto-update" / "parent app waiting on dependency" / "can't add another superseded app" → `Troubleshooting/Win32AppRelationships-B.md` + `Scripts/Get-Win32AppRelationshipAudit.ps1` (deep dive: `Win32AppRelationships-A.md`)
...
- "Vulnerability Remediation Agent won't set up / Run button greyed out / agent can't run anymore" → `Troubleshooting/VulnerabilityRemediationAgent-B.md` Fix 1-3 — check licensing/cloud type first, then Run Readiness Check, then the 90-day human-identity expiration
- "Agent shows zero or very low findings despite a large fleet" → `Troubleshooting/VulnerabilityRemediationAgent-B.md` Fix 4 — confirm Windows Server devices aren't the population being checked (structurally excluded)
...
- "Activate eSIM / Remove eSIM button missing on an Android device" → `Troubleshooting/AndroidESIM-B.md` Fix 1 — confirm Preview new device view is on and the device is corporate-owned, not BYOD
- "eSIM activation failed with a Google/carrier error on an Android device" → `Troubleshooting/AndroidESIM-B.md` Fix 2 — Intune doesn't pre-check slot capacity, so this is very often a genuine device/carrier-side rejection
- "Remove eSIM works on some Android devices but not others at the same OS version" → `Troubleshooting/AndroidESIM-B.md` Fix 4 — check exact ownership sub-type; COPE needs Android 17+ for Remove, COBO/COSU only need 15+
- "Cloud PC named CPCA-... shows blank details" / "agent pool says Failed/Available with warning" / "policy edit for an agent pool didn't do anything" → `Azure/Windows365/Agents-B.md` Triage (pool-based, not device-based — check pool status and Preview new device view first)
- "Samsung devices not showing up in Knox E-FOTA" / "firmware campaign stuck at 0%" / "corporate Android device won't register for firmware updates" → `Troubleshooting/SamsungKnoxEFOTA-B.md` + `Scripts/Get-KnoxEFOTAReadinessAudit.ps1` (check platform eligibility — COBO/COSU/COPE only — then apps/OEMConfig, then whether the end user actually opened the Knox E-FOTA app to accept terms)
- "Registry value we configured to collect never shows up in Device Inventory" / "properties catalog policy shows Succeeded but no data" → `Troubleshooting/PropertiesCatalog-B.md` Fix 1-2 — check the 24h initial-collection window first, then HKLM-only hive and 6KB size limits, then silent sensitive-value filtering (no override exists)
- "Deployment paused in error state" / "deployment disappeared after I created it" / "cancelled the deployment but the app is still installing" / "final ring wiped our original assignments" → `Troubleshooting/DeploymentPlans-B.md`: collision, deleted-group, Multi Admin Approval, and by-design cumulative/replace assignment behaviour of Intune Deployments (preview)
- "Want to stage a Win32 app / Settings catalog policy out in rings" → `Troubleshooting/DeploymentPlans-A.md` Playbook 4, then run `Scripts/Get-IntuneDeploymentReadiness.ps1` against the payload and planned ring groups before creating the deployment
- "Need to stop collecting one registry key but keep the rest" → `Troubleshooting/PropertiesCatalog-B.md` Fix 4 — not supported; collection can only be stopped at the category level
- "Device Query tab missing / query times out / multi-device query shows no Windows devices / KQL error on Device" → `Troubleshooting/DeviceQuery-B.md` (run `Scripts/Get-DeviceQueryReadiness.ps1` on the device first)
- "Anomalies report empty / App reliability tab disappeared / Device timeline missing / battery or resource performance report blank / device scope Insufficient Data" → `Troubleshooting/AdvancedAnalytics-B.md` (run `Scripts/Get-AdvancedAnalyticsReportAudit.ps1` for tenant-side data)
- "Microsoft Tunnel server offline / not checking in / users can't connect after Linux reboot" → `Troubleshooting/MicrosoftTunnel-B.md` (Triage, Fixes 1-3)
- "Tunnel connects but internal sites unreachable" → `Troubleshooting/MicrosoftTunnel-B.md` (Fix 3 kernel prereqs, Fix 5 DNS/routes/bridge overlap)
- "Tunnel TLS certificate expiring / iOS rejects cert" → `Troubleshooting/MicrosoftTunnel-B.md` (Fix 4)
- "Health check says Offline but users still connect" → `Troubleshooting/MicrosoftTunnel-B.md` (Fix 7 — documented known issue, reinstall re-enrolls)
- "Fleet health snapshot of all Tunnel Sites/servers" → `Scripts/Get-MicrosoftTunnelHealthAudit.ps1`
- "Edge setting set in the M365 admin center / Edge management service isn't applying" → `Troubleshooting/EdgeManagement-B.md` (Fix 1, Fix 3 precedence)
- "Edge policy shows Conflict / wrong value wins between GPO, Intune and cloud" → `Troubleshooting/EdgeManagement-B.md` (Fix 3) + `Scripts/Get-EdgePolicySourceAudit.ps1`
- "MSP/GDAP admin can't manage Microsoft Edge settings" → `Troubleshooting/EdgeManagement-B.md` (Fix 6)
- "Cloud vs Intune policy type in Edge management service — which to use?" → `Troubleshooting/EdgeManagement-A.md` (How It Works)
- "Android device went noncompliant overnight, nobody changed anything" / "app protection policy suddenly blocking access on Android" → `Troubleshooting/AndroidPlayIntegrity-B.md` Triage — check `AndroidSecurityPatchLevel` against the 12-month window before assuming a policy or Intune bug
- "Strong integrity check fails on some Android models but not others at the same patch level" → `Troubleshooting/AndroidPlayIntegrity-B.md` Fix 2 — likely a hardware attestation ceiling, not a patchable issue
- "Getting ready for the Oct 31, 2026 Play Integrity enforcement date" → `Scripts/Get-AndroidPlayIntegrityAudit.ps1` — run proactively, not reactively, per `Troubleshooting/AndroidPlayIntegrity-B.md` Fix 4
- "User can't add a second work account in Outlook/Teams on iPhone" / "second managed account keeps getting rejected" → `Troubleshooting/AppMultipleManagedAccounts-B.md` Triage — check app version against the MMA floor first, then the `IntuneMAMAllowedAccountsOnly` app-config key
- "Copy/paste or screenshots suddenly blocked in Outlook combined inbox even though the account's own policy allows it" → `Troubleshooting/AppMultipleManagedAccounts-B.md` Fix 3 — mixed-view full lockdown is expected behavior, not a policy regression
