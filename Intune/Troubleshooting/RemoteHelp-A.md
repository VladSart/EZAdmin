...
**Update (Intune Suite Service Release 2608, August 2026):** this table reflects the *attended* helper/sharer mode matrix only. Windows unattended access shipped as a **separate feature, app stack, and RBAC permission** — "Windows Unattended Support with Remote Sign-In" — that does not fit this table's helper/sharer pairing model at all, since it connects a helper directly to a device's login screen rather than pairing with a sharer's client session. It is covered in full in `RemoteHelp-Unattended-A.md`/`-B.md`, including its own dependency stack (Azure Virtual Desktop Agent + Bootloader, not `RemoteHelp.exe`) and its own dedicated permission (`Remote Help app - Windows unattended control remote sign-in`, distinct from the `Remote Help — Unattended` row below, which remains Android-dedicated-device-only). Do not conflate the two "unattended" capabilities — they are unrelated implementations that happen to share a name.
...
| Remote Help — Unattended | Connect to Android dedicated devices without per-session sharer acceptance (Android-only; the unrelated Windows equivalent is a separate permission — see below) |
...
- **Update:** the *attended* mode-support table's "Unattended" column remains Android-dedicated-device-only, as originally noted here — but as of Intune Suite Service Release 2608 (August 2026), a separate Windows unattended capability now exists outside this table entirely (its own app stack, permission, and session model). See `RemoteHelp-Unattended-A.md`/`-B.md` for that capability specifically, and re-check both this table and that document at time of engagement — this remains an actively evolving area of the product. See: [Helper and client modes](https://learn.microsoft.com/en-us/intune/remote-help/plan#helper-and-client-modes)
