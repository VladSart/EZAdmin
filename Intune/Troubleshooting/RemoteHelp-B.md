...
- **Update:** the attended-session mode-support matrix's "Unattended" column remains Android-dedicated-device-only, as this note originally said. But as of Intune Suite Service Release 2608 (August 2026), Windows now has its own separate unattended capability — a different app stack, permission, and ticket-triage path entirely. See `RemoteHelp-Unattended-B.md` for that scenario specifically; don't troubleshoot a Windows unattended-access ticket against this file's dependency chain. See [Helper and client modes](https://learn.microsoft.com/en-us/intune/remote-help/plan#helper-and-client-modes).
...
- **Starting 1 July 2026, Remote Help is bundled into Microsoft 365 E3 and E5** (previously a separate Intune Suite add-on or standalone Remote Help purchase) — a tenant may show Remote Help as licensed for both helper and sharer with no separate purchase, which can surprise an MSP still tracking it as paid add-on spend. See `IntuneSuiteBaseLicensing-B.md` for the full licensing triage flow before troubleshooting a "Remote Help unlicensed" ticket as a technical fault. [Advanced Microsoft Intune capabilities now available in Microsoft 365 E3 and E5](https://techcommunity.microsoft.com/blog/microsoftintuneblog/advanced-microsoft-intune-capabilities-now-available-in-microsoft-365-e3-and-e5/4529335)
