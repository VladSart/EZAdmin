...
**Incoming change (MC1450128 / Roadmap 568785):** starting with Public Preview in mid-September 2026 (GA mid-October–mid-November 2026), adaptive scopes gain explicit lifecycle-status evaluation controls. Newly created scopes will evaluate **active recipients only by default** — a departed employee's inactive mailbox will silently drop out of scope (and stop being monitored) unless the scope is explicitly configured to include inactive/soft-deleted accounts. For Communication Compliance specifically, this matters most for offboarding scenarios where monitoring needs to continue through a notice period on an account that's since gone inactive — confirm scope configuration explicitly rather than assuming continued coverage. Existing scopes are not modified by the rollout. [Microsoft Learn — Adaptive scopes](https://learn.microsoft.com/en-us/purview/purview-adaptive-scopes)
...
- **Adaptive scopes are gaining lifecycle-status controls (MC1450128, Preview mid-Sep 2026 / GA mid-Oct 2026)** that default new scopes to active-recipients-only. This is a meaningful gap for offboarding-adjacent monitoring use cases — a scope built to watch a flight-risk or under-investigation employee's communications must be explicitly configured to keep including them once their mailbox goes inactive. [Microsoft Learn — Adaptive scopes](https://learn.microsoft.com/en-us/purview/purview-adaptive-scopes)
