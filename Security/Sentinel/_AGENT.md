...
| `MDTIRetirement-B.md` | Hotfix runbook — disambiguating the retired standalone MDTI portal/product from the Defender-portal TI experience and from this folder's own Sentinel TI ingestion layer, dead-bookmark redirects, missing Intel profiles/Intel explorer (license gate), CSP/MSP stale-billing cleanup, broken standalone-API integrations |
| `MDTIRetirement-A.md` | Deep dive — the three-things-share-one-name architecture, the 2025-10-01 purchasable-SKU removal vs. 2026-08-01 portal shutdown timeline (plus a flagged conflicting community-post date), the licensing convergence into E5/E5-Security-add-on/Defender-for-Endpoint-P2/free-Sentinel-connector, and the CSP credit-memo billing tail |
| `Scripts/Get-MDTIRetirementAudit.ps1` | Audits tenant/user licensing coverage for the post-retirement research experience (qualifying-SKU inventory, per-user assignment gaps) and optionally checks for a Sentinel TI connector as an alternate free access path — read-only, no standalone-MDTI surface exists to check directly |
...
- "Our bookmarked MDTI portal link stopped working" → `MDTIRetirement-B.md` Fix 1 (standalone portal retired 2026-08-01, expected)
- "Analyst can't see Intel profiles/Intel explorer in the Defender portal" → `MDTIRetirement-B.md` Fix 2 (license gate — E5/E5 Security add-on/Defender for Endpoint P2/Sentinel connector)
- "Is this MDTI ticket actually about our Sentinel threat intel feed instead?" → `MDTIRetirement-B.md` Triage disambiguation (three different "MDTI" things share the name — check which one first)
- "Client still being billed for standalone MDTI" → `MDTIRetirement-B.md` Fix 4 / `MDTIRetirement-A.md` Playbook 1 (CSP credit memo lands with the partner, not the customer)
