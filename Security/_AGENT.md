...
| `Defender/` | The largest sub-product — Defender for Endpoint (MDE) onboarding/sensor health, Defender for Cloud Apps (MDA) and its File Policy retirement (Jan 6, 2027), Defender for Identity (MDI), Defender for Cloud (CSPM) and CIEM, Defender Vulnerability Management, ASR rules, Network/Cloud/Tamper Protection, WDAC, Automated Investigation & Response (AIR), Predictive shielding (Preview — proactive GPO/SafeBoot hardening and account containment on predicted-next-attack-targets, an extension of automatic attack disruption), App Governance, Device Control, Attack Simulation Training, Defender for Office 365 Safe Links/Safe Attachments, Defender for Business, and the tenant-wide Secure Score |
...
- "GPOs stopped applying / device won't boot to Safe Mode, no admin change made" → `Defender/PredictiveShielding-B.md` (Preview — Defender autonomously hardened a predicted next-attack-target; check the originating incident before assuming misconfiguration)
