...
- **Authentication Context vs. Authentication Strength are different, complementary controls.** Authentication Context (`c1`–`c99` claims tagged on specific apps/actions, e.g., a SharePoint sensitivity label) triggers *when* a step-up challenge fires; Authentication Strength defines *what counts* as satisfying it. Don't confuse the two when a client asks for "step-up MFA on sensitive documents." See `AuthenticationContext-B.md`/`-A.md`. [Authentication context](https://learn.microsoft.com/en-us/entra/identity/conditional-access/concept-conditional-access-cloud-apps#authentication-context)
